...
type IndependentPracticeSessionType =
...
type IndependentPracticeReviewContext = {
  sessionId: string;
  sessionType: IndependentPracticeSessionType;
  mode: ReviewMode;
...
const BUILD_LABEL = "Independent Practice Review v1 Alpha";
...
function PulleyDebriefScreen({ journey, stage, onWhy, onDashboard, onNext, onReviewIncorrect, onReviewAll }: { journey: MvpGuestJourney; stage: "guided" | "independent" | "assessment"; onWhy: () => void; onDashboard: () => void; onNext?: () => void; onReviewIncorrect?: () => void; onReviewAll?: () => void }) {
  ...
  const latestIndependentSession = stage === "independent" ? getLatestSessionOfType(journey, "pulley_independent_practice") : undefined;
  const incorrectCount = latestIndependentSession ? journey.responses.filter((response) => response.sessionId === latestIndependentSession.sessionId && !response.correct).length : 0;
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-4xl items-center px-8 py-16"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{stage === "assessment" ? "Pulley Check debrief" : "Pulley practice debrief"}</p><h1 className="mt-6 text-4xl font-semibold leading-tight">{debrief?.title}</h1><p className="mt-6 text-lg leading-relaxed text-[#9AA3B2]">{debrief?.summary}</p>{summary && <div className="mt-8 grid gap-4 sm:grid-cols-3">{summary.conceptBreakdown.map((item)=><div key={item.concept} className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">{labels[item.concept] ?? item.concept}</div><div className="mt-3 text-3xl font-semibold">{item.correct}/{item.attempted}</div><div className="mt-2 text-sm text-[#9AA3B2]">{Math.round(item.accuracy * 100)}%</div></div>)}</div>}<div className="mt-8 grid gap-5"><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Interpretation</div><p className="mt-3 text-[#C8D2DD]">{debrief?.interpretation}</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Recommended next step</div><h2 className="mt-3 text-2xl font-semibold">{rec?.title}</h2><p className="mt-3 text-[#AAB4C0]">{rec?.summary}</p></div></div>{stage === "independent" && onReviewIncorrect && onReviewAll && <IndependentPracticeReviewActions incorrectCount={incorrectCount} onReviewIncorrect={onReviewIncorrect} onReviewAll={onReviewAll} />}<div className="mt-9 flex flex-col gap-3 sm:flex-row"><SecondaryButton onClick={onWhy}>Why this next step?</SecondaryButton>{canUseNext && onNext && <PrimaryButton onClick={onNext}>{stage === "guided" ? "Start Independent Pulley Practice" : "Start Pulley Check"}</PrimaryButton>}<PrimaryButton onClick={onDashboard}>View dashboard</PrimaryButton></div></Card></section></Shell>;
}
...
function LeverDebriefScreen({ journey, stage, onWhy, onDashboard, onNext, onReviewIncorrect, onReviewAll }: { journey: MvpGuestJourney; stage: "guided" | "independent" | "assessment"; onWhy: () => void; onDashboard: () => void; onNext?: () => void; onReviewIncorrect?: () => void; onReviewAll?: () => void }) {
  ...
  const latestIndependentSession = stage === "independent" ? getLatestSessionOfType(journey, "lever_independent_practice") : undefined;
  const incorrectCount = latestIndependentSession ? journey.responses.filter((response) => response.sessionId === latestIndependentSession.sessionId && !response.correct).length : 0;
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-4xl items-center px-8 py-16"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{stage === "assessment" ? "Lever Check debrief" : "Lever practice debrief"}</p><h1 className="mt-6 text-4xl font-semibold leading-tight">{debrief?.title}</h1><p className="mt-6 text-lg leading-relaxed text-[#9AA3B2]">{debrief?.summary}</p>{summary && <div className="mt-8 grid gap-4 sm:grid-cols-3">{summary.conceptBreakdown.map((item)=><div key={item.concept} className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">{labels[item.concept] ?? item.concept}</div><div className="mt-3 text-3xl font-semibold">{item.correct}/{item.attempted}</div><div className="mt-2 text-sm text-[#9AA3B2]">{Math.round(item.accuracy * 100)}%</div></div>)}</div>}<div className="mt-8 grid gap-5"><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Interpretation</div><p className="mt-3 text-[#C8D2DD]">{debrief?.interpretation}</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Recommended next step</div><h2 className="mt-3 text-2xl font-semibold">{rec?.title}</h2><p className="mt-3 text-[#AAB4C0]">{rec?.summary}</p></div></div>{stage === "independent" && onReviewIncorrect && onReviewAll && <IndependentPracticeReviewActions incorrectCount={incorrectCount} onReviewIncorrect={onReviewIncorrect} onReviewAll={onReviewAll} />}<div className="mt-9 flex flex-col gap-3 sm:flex-row"><SecondaryButton onClick={onWhy}>Why this next step?</SecondaryButton>{canUseNext && onNext && <PrimaryButton onClick={onNext}>{stage === "guided" ? "Start Independent Lever Practice" : "Start Lever Check"}</PrimaryButton>}<PrimaryButton onClick={onDashboard}>View dashboard</PrimaryButton></div></Card></section></Shell>;
}
...
function AbstractLogicalPracticeIntroScreen({ stage, onStart }: { stage: AbstractLogicalStage; onStart: () => void }) {
  const config = stage === "guided"
    ...
    : stage === "independent"
      ...
      : { eyebrow: "Assessment-style check", title: "Abstract & Logical Check", body: "Twelve mixed questions with no immediate answer feedback. The Mentor will identify either a clear weakness or a progression step.", items: ["12 questions", "No immediate feedback", "Untimed in v1"], button: "Start Abstract & Logical Check" };
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-3xl items-center px-8 py-16"><Card className="text-center"><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{config.eyebrow}</p><h1 className="mt-5 text-4xl font-semibold">{config.title}</h1><p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-[#9AA3B2]">{config.body}</p><div className="mx-auto mt-9 grid max-w-xl gap-3 sm:grid-cols-3">{config.items.map((item) => <div key={item} className="rounded-xl border border-white/5 bg-[#111418] p-4 text-sm text-[#AAB4C0]">{item}</div>)}</div><div className="mt-10"><PrimaryButton onClick={onStart}>{config.button}</PrimaryButton></div></Card></section></Shell>;
}
...
function VerbalPracticeIntroScreen({ stage, onStart }: { stage: VerbalStage; onStart: () => void }) {
  const config = stage === "guided"
    ...
    : stage === "independent"
      ...
      : { eyebrow: "Assessment-style check", title: "Verbal Comprehension Check", body: "Twelve mixed questions with no immediate answer feedback. The Mentor will identify either a clear weakness or a progression step.", items: ["12 questions", "No immediate feedback", "Untimed in v1"], button: "Start Verbal Check" };
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-3xl items-center px-8 py-16"><Card className="text-center"><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{config.eyebrow}</p><h1 className="mt-5 text-4xl font-semibold">{config.title}</h1><p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-[#9AA3B2]">{config.body}</p><div className="mx-auto mt-9 grid max-w-xl gap-3 sm:grid-cols-3">{config.items.map((item) => <div key={item} className="rounded-xl border border-white/5 bg-[#111418] p-4 text-sm text-[#AAB4C0]">{item}</div>)}</div><div className="mt-10"><PrimaryButton onClick={onStart}>{config.button}</PrimaryButton></div></Card></section></Shell>;
}
...
function VerbalDebriefScreen({ journey, stage, onWhy, onDashboard, onNext, onReviewIncorrect, onReviewAll }: { journey: MvpGuestJourney; stage: VerbalStage; onWhy: () => void; onDashboard: () => void; onNext?: () => void; onReviewIncorrect?: () => void; onReviewAll?: () => void }) {
  ...
  const latestIndependentSession = stage === "independent" ? getLatestSessionOfType(journey, "verbal_independent_practice") : undefined;
  const incorrectCount = latestIndependentSession ? journey.responses.filter((response) => response.sessionId === latestIndependentSession.sessionId && !response.correct).length : 0;
  const isProgression = stage === "assessment" && rec?.recommendationType === "continue_verbal_practice";
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-5xl items-center px-8 py-16"><Card><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{stage === "assessment" ? "Verbal Comprehension Check debrief" : "Verbal practice debrief"}</p><h1 className="mt-6 text-4xl font-semibold leading-tight">{debrief?.title}</h1><p className="mt-6 text-lg leading-relaxed text-[#9AA3B2]">{debrief?.summary}</p></div>{stage === "assessment" && <Badge>{isProgression ? "Progression recommendation" : "Weakness recommendation"}</Badge>}</div>{summary && <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{summary.conceptBreakdown.map((item) => <div key={item.concept} className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">{verbalCategoryLabels[item.concept as VerbalSubcompetency] ?? item.concept}</div><div className="mt-3 text-3xl font-semibold">{item.correct}/{item.attempted}</div><div className="mt-2 text-sm text-[#9AA3B2]">{Math.round(item.accuracy * 100)}%</div></div>)}</div>}<div className="mt-8 grid gap-5 lg:grid-cols-2"><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Interpretation</div><p className="mt-3 leading-relaxed text-[#C8D2DD]">{debrief?.interpretation}</p></div><div className="rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/5 p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Mentor recommendation</div><h2 className="mt-3 text-2xl font-semibold">{rec?.title}</h2><p className="mt-3 leading-relaxed text-[#AAB4C0]">{rec?.summary}</p></div></div>{stage === "independent" && onReviewIncorrect && onReviewAll && <IndependentPracticeReviewActions incorrectCount={incorrectCount} onReviewIncorrect={onReviewIncorrect} onReviewAll={onReviewAll} />}<div className="mt-9 flex flex-col gap-3 sm:flex-row"><SecondaryButton onClick={onWhy}>Why this recommendation?</SecondaryButton>{canUseNext && onNext && <PrimaryButton onClick={onNext}>{stage === "guided" ? "Start Independent Verbal Practice" : "Start Verbal Check"}</PrimaryButton>}<PrimaryButton onClick={onDashboard}>View dashboard</PrimaryButton></div></Card></section></Shell>;
}


const independentPracticeReviewConfig: Record<IndependentPracticeSessionType, { title: string; questions: MvpQuestion[] }> = {
...
function getLatestSessionOfType(journey: MvpGuestJourney, sessionType: IndependentPracticeSessionType) {
  return [...journey.sessions].reverse().find((session) => session.sessionType === sessionType);
}

function IndependentPracticeReviewActions({ incorrectCount, onReviewIncorrect, onReviewAll }: { incorrectCount: number; onReviewIncorrect: () => void; onReviewAll: () => void }) {
  return (
...
function IndependentPracticeReviewVisual({ question, sessionType }: { question: MvpQuestion; sessionType: IndependentPracticeSessionType }) {
  ...
  if (sessionType === "numerical_independent_practice") return <NumericalDataTableView table={question.dataTable} />;
  if (sessionType === "abstract_logical_independent_practice" && question.abstractVisual) return <AbstractLogicalVisualPanel visual={question.abstractVisual} />;
  if (sessionType === "verbal_independent_practice") return <VerbalPassagePanel passage={question.verbalPassage} />;
  return null;
}

function IndependentPracticeReviewScreen({ journey, context, onModeChange, onBack }: { journey: MvpGuestJourney; context: IndependentPracticeReviewContext; onModeChange: (mode: ReviewMode) => void; onBack: () => void }) {
  const [reviewIndex, setReviewIndex] = useState(0);
  useEffect(() => { setReviewIndex(0); window.scrollTo({ top: 0, behavior: "smooth" }); }, [context.sessionId, context.mode]);
  const config = independentPracticeReviewConfig[context.sessionType];
  ...
  return (
    <Shell right="Answer review">
      <section className="mx-auto max-w-5xl px-8 py-12">
        ...
        <Card>
          <IndependentPracticeReviewVisual question={current.question} sessionType={context.sessionType} />
...
export default function FloSpatialPrototype() {
  ...
  const [reviewContext, setReviewContext] = useState<IndependentPracticeReviewContext | null>(null);
  ...
  function openIndependentPracticeReview(sessionType: IndependentPracticeSessionType, mode: ReviewMode, returnScreen: AppScreen) {
    ...
    setShowWhy(false);
    setScreen("independent-practice-review");
  }
  function changeIndependentPracticeReviewMode(mode: ReviewMode) {
    setReviewContext((current) => current ? { ...current, mode } : current);
  }
  function closeIndependentPracticeReview() {
