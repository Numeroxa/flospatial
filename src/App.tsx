...
import { VERBAL_CALIBRATION_PILOT_VERSION, verbalCalibrationAllItems, verbalCalibrationCoreItems, type VerbalCalibrationOptionId, type VerbalCalibrationPilotItem, type VerbalCalibrationSource } from "./verbalCalibrationCatalog";
...
function downloadVerbalCalibrationPilotCsv() {
  ...
  runs.forEach((run) => run.responses.forEach((response) => {
    ...
    rows.push([
      "APTESTA_VERBAL_CAL_EXPORT_V0_15", run.pilotVersion, run.runId, response.questionId, item?.blueprintId, item?.familyId, item?.archetype, item?.difficulty, item?.reasoningSteps, item?.targetTimeRangeSec.minSec, item?.targetTimeRangeSec.maxSec, response.selectedOptionId, response.correct, Math.round(response.selectionTimeMs), response.deviceClass, response.misconceptionTag, response.answeredAt,
    ]);
  }));
  const csv = rows.map((row) => row.map(csvCell).join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
...
function VerbalCalibrationSourceView({ source }: { source: VerbalCalibrationSource }) {
  return <div className="mb-6 rounded-2xl border border-white/5 bg-[#0F1216] p-5 sm:p-6">
...
function InternalVerbalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  ...
  const [runStartedAt, setRunStartedAt] = useState<string | null>(null);
  const questionStartedAtRef = useRef(0);
  const activeItems = verbalCalibrationCoreItems;
  ...
  if (phase === "intro") {
    ...
    return <Shell right="Internal verbal calibration"><section className="mx-auto max-w-5xl px-4 py-8 sm:px-8 sm:py-12"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only verbal lab · v0.15</p><h1 className="mt-4 text-4xl font-semibold">Verbal calibration</h1><p className="mt-5 max-w-3xl leading-relaxed text-[#9AA3B2]">This first verbal slice contains eight short, practical reading tasks. The priority is clean comprehension on a phone: enough text to test the intended skill, but no unnecessary reading load or hidden specialist knowledge.</p><div className="mt-7 rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">New v0.15 QA set</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Core comprehension</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · explicit fact, condition, main point, supported inference, meaning in context, instruction sequence, exception handling and evidence scope.</div><PrimaryButton className="mt-5" onClick={startRun}>Start 8-item v0.15 QA set</PrimaryButton></div><div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-5 text-sm leading-relaxed text-[#C8D2DD]">For device QA, check passage comfort on iPhone, whether the wording has only one defensible answer, whether distractors are plausible without being tricky, and whether you can move between passage, question and answers without losing your place.</div><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton>{priorRuns.length > 0 && <SecondaryButton onClick={downloadVerbalCalibrationPilotCsv}>Export {priorRuns.length} prior run{priorRuns.length === 1 ? "" : "s"}</SecondaryButton>}</div></Card></section></Shell>;
  }
