...
const GEAR_FLUENCY_INITIAL_TARGET_MS = 35000;
...
function GearFluencyQuestionScreen({ journey, sessionId, questionIndex, onAnswer }: { journey: MvpGuestJourney; sessionId: string; questionIndex: number; onAnswer: (response: AssessmentResponse, final: boolean) => void }) {
  ...
  useEffect(() => { window.scrollTo({ top: 0, behavior: "smooth" }); const start = Date.now(); setStartedAt(start); setElapsedMs(0); setSelectedOptionId(null); setShowFeedback(false); const timer = window.setInterval(() => setElapsedMs(Date.now() - start), 250); return () => window.clearInterval(timer); }, [questionIndex]);
  ...
  function next() { if (!selectedOptionId) return; const response = createAssessmentResponse(sessionId, question, selectedOptionId, Date.now() - startedAt, false); onAnswer(response, questionIndex === gearFluencyQuestions.length - 1); }
  const paceLabel = elapsedMs <= GEAR_FLUENCY_INITIAL_TARGET_MS ? "Within the gentle target" : "No problem — keep the method accurate";
  return <Shell right="Gear fluency"><section className="mx-auto max-w-5xl px-6 pb-20 pt-8 sm:px-8 sm:pb-12 sm:pt-10"><div className="mb-5 h-1.5 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-[#5ED3F3]/70 transition-all" style={{ width: `${progress}%` }} /></div><div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Gear Direction Fluency</p><h1 className="mt-3 text-3xl font-semibold">Recognise the pattern</h1></div><div className="text-right text-sm text-[#8D98A6]">Question {questionIndex + 1} of {gearFluencyQuestions.length}<br/><span className="text-xs">{formatSeconds(elapsedMs)} · {paceLabel}</span></div></div><div className="mb-5 rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 px-4 py-3 sm:px-5"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#5ED3F3]">Fluency cue</p><p className="mt-1 text-sm leading-relaxed text-[#C8D2DD]">Count contacts → even = same direction · odd = opposite direction.</p></div><Card><GearQuestionDiagram question={question} mode="practice" /><p className="mt-6 text-xl leading-relaxed text-[#F4F6F8]">{question.stem}</p><div className="mt-6 grid gap-3">{visibleOptions.map((option) => <button key={option.optionId} type="button" onClick={() => select(option.optionId)} disabled={showFeedback} className={`rounded-2xl border bg-[#111418] p-5 text-left transition ${selectedOptionId === option.optionId ? "border-[#5ED3F3]/60 bg-[#5ED3F3]/10" : "border-white/10 hover:border-[#5ED3F3]/40"} ${showFeedback ? "cursor-default" : ""}`}><span className="mr-3 text-[#5ED3F3]">{option.label}</span><span className="text-[#DCE3EA]">{option.text}</span></button>)}</div>{showFeedback && <div className={`mt-5 rounded-2xl border p-5 ${selectedCorrect ? "border-[#38D39F]/40 bg-[#101D1A]" : "border-[#FFB86B]/40 bg-[#211813]"}`}><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-semibold">{selectedCorrect ? "Correct" : "Not quite"}</p><p className="mt-2 leading-relaxed text-[#C8D2DD]">{question.explanation}</p><p className="mt-3 text-sm text-[#8D98A6]">This answer took {formatSeconds(Date.now() - startedAt)}. The goal is reliable recognition; speed comes gradually.</p></div><div className="shrink-0"><PrimaryButton className="w-full sm:w-auto" onClick={next}>{questionIndex === gearFluencyQuestions.length - 1 ? "Complete fluency set" : "Next question"}</PrimaryButton></div></div></div>}</Card></section></Shell>;
}
