...
import { VERBAL_CALIBRATION_PILOT_VERSION, verbalCalibrationAllItems, verbalCalibrationAppliedItems, verbalCalibrationCoreItems, type VerbalCalibrationOptionId, type VerbalCalibrationPilotItem, type VerbalCalibrationSource } from "./verbalCalibrationCatalog";
...
function InternalVerbalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  ...
  const [runStartedAt, setRunStartedAt] = useState<string | null>(null);
  const [pilotSet, setPilotSet] = useState<"core" | "applied">("applied");
  const questionStartedAtRef = useRef(0);
  const activeItems = pilotSet === "core" ? verbalCalibrationCoreItems : verbalCalibrationAppliedItems;
  ...
  function startRun(set: "core" | "applied" = "applied") {
    setPilotSet(set);
    setQuestionIndex(0);
    setSelectedOptionId(null);
    setResponses([]);
  ...
  if (phase === "intro") {
    ...
    return <Shell right="Internal verbal calibration"><section className="mx-auto max-w-5xl px-4 py-8 sm:px-8 sm:py-12"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only verbal lab · v0.16</p><h1 className="mt-4 text-4xl font-semibold">Verbal calibration</h1><p className="mt-5 max-w-3xl leading-relaxed text-[#9AA3B2]">The first eight Verbal items are now QA-cleared. This second slice adds longer reading, mixed-source interpretation and faster vocabulary/relationship tasks while preserving the same phone-first reading layout.</p><div className="mt-7 grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-white/5 bg-[#111418] p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Core comprehension</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 01–08. QA-cleared after device review.</div><SecondaryButton className="mt-5" onClick={() => startRun("core")}>Re-run 01–08</SecondaryButton></div><div className="rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">New v0.16 QA set</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Extended verbal reasoning</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · compare two texts, longer-passage fact and inference, table + text, antonym, synonym, word relationship and sentence completion.</div><PrimaryButton className="mt-5" onClick={() => startRun("applied")}>Start 8-item v0.16 QA set</PrimaryButton></div></div><div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-5 text-sm leading-relaxed text-[#C8D2DD]">For device QA, pay particular attention to the longer passage on Questions 2–3, the two-notice comparison on Question 1, and whether the Question 4 roster table remains fully visible on iPhone. For Questions 5–8, check that the faster verbal items feel clear rather than trivial or vocabulary-dependent.</div><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton>{priorRuns.length > 0 && <SecondaryButton onClick={downloadVerbalCalibrationPilotCsv}>Export {priorRuns.length} prior run{priorRuns.length === 1 ? "" : "s"}</SecondaryButton>}</div></Card></section></Shell>;
  }
