...
type MvpQuestion = {
  questionId: string;
  sessionType: "mechanical_starting_point" | "guided_hydraulic_practice";
  pathwayId: PathwayId;
  domain: "mechanical";
...
type LearningModuleId = "hydraulic_fundamentals";
...
type PracticeSummary = {
  summaryId: string;
  sessionId: string;
  sessionType: "guided_hydraulic_practice";
  attempted: number;
  correct: number;
  accuracy: number;
...
function completeGuidedHydraulicPractice(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  if (strongImprovement) {
    recommendationType = "start_applied_hydraulic_problems";
    title = "Start Applied Hydraulic Problems";
    summaryText = "Check whether your hydraulic improvement transfers into less guided, more realistic problems.";
    actionLabel = "Start Applied Problems";
    ...
    whyTitle = "Why Applied Hydraulic Problems is recommended";
    ...
    recommendationText = "Applied Hydraulic Problems is recommended to test that transfer.";
    ...
    debriefInterpretation = "This suggests the foundation module may have addressed part of the earlier hydraulic-force reasoning constraint.";
  ...
  const improvementMilestone: Milestone | null = recommendationType !== "review_hydraulic_fundamentals" ? { milestoneId: id("milestone"), type: "first_improvement_signal", label: recommendationType === "start_applied_hydraulic_problems" ? "First improvement signal detected" : "Early improvement signal detected", createdAt: now() } : null;
  ...
  return {
    ...
    dashboardState: { ...(journey.dashboardState ?? { dashboardStateId: id("dash"), recentMilestoneIds: [], saveStatus: "local_only" as const, updatedAt: now() }), currentRecommendationId: recommendation.recommendationId, currentFocusLabel: recommendationType === "start_applied_hydraulic_problems" ? "Applied hydraulic reasoning" : "Hydraulic-force reasoning", readinessSnapshotId: readiness.readinessSnapshotId, recentMilestoneIds, baselineSummary: journey.dashboardState?.baselineSummary, saveStatus: "local_only", updatedAt: now() },
    updatedAt: now(),
  };
}
...
function TestScenarioPanel({ onLoad }: { onLoad: (scenario: TestScenario) => void }) {
  const scenarios: { scenario: TestScenario; title: string; description: string }[] = [
    ...
    },
    {
      ...
      description: "Loads a journey where the next step is Applied Hydraulic Problems.",
    },
    {
...
function DashboardScreen({ journey, onWhy, onReset, onStartHydraulics, onStartGuidedPractice, onLoadTestScenario }: { journey: MvpGuestJourney; onWhy: () => void; onReset: () => void; onStartHydraulics: () => void; onStartGuidedPractice: () => void; onLoadTestScenario: (scenario: TestScenario) => void }) {
