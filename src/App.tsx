...
import { ABSTRACT_LOGICAL_CALIBRATION_PILOT_VERSION, abstractLogicalCalibrationAllItems, abstractLogicalCalibrationClassificationDeductionItems, abstractLogicalCalibrationCompletionItems, abstractLogicalCalibrationSequenceMatrixItems, type AbstractLogicalCalibrationDiagram, type AbstractLogicalCalibrationOptionId, type AbstractLogicalCalibrationPilotItem, type AbstractLogicalCell, type AbstractLogicalMark } from "./abstractLogicalCalibrationCatalog";
import { VERBAL_CALIBRATION_PILOT_VERSION, verbalCalibrationAllItems, verbalCalibrationAppliedItems, verbalCalibrationCompletionItems, verbalCalibrationCoreItems, type VerbalCalibrationOptionId, type VerbalCalibrationPilotItem, type VerbalCalibrationSource } from "./verbalCalibrationCatalog";
...
type AppScreen =
  ...
  | "dashboard"
  | "internal-fluency-analytics"
  | "internal-calibration-framework"
  | "internal-calibration-readiness"
  | "internal-mechanical-calibration-pilot"
  | "internal-numerical-calibration-pilot"
  | "internal-abstract-logical-calibration-pilot"
  | "internal-verbal-calibration-pilot"
  ...
  | "gear-fluency-intro"
  | "gear-fluency-question"
  | "gear-fluency-debrief"
...
type MvpQuestion = {
  questionId: string;
  sessionType: "mechanical_starting_point" | "guided_hydraulic_practice" | "hydraulic_independent_practice" | "mixed_mechanical_practice" | "guided_gear_practice" | "gear_independent_practice" | "gear_fluency" | "gear_assessment" | "guided_pulley_practice" | "pulley_independent_practice" | "pulley_assessment" | "guided_lever_practice" | "lever_independent_practice" | "lever_assessment" | "mixed_mechanical_assessment" | "guided_numerical_practice" | "numerical_independent_practice" | "numerical_assessment" | "guided_abstract_logical_practice" | "abstract_logical_independent_practice" | "abstract_logical_assessment" | "guided_verbal_practice" | "verbal_independent_practice" | "verbal_assessment";
  pathwayId: PathwayId;
  domain: Domain;
...
type AssessmentResponse = {
  ...
  answeredAt: string;
  responseTimeBasis?: "selection_latency" | "legacy_next_click";
  firstActionTimeMs?: number;
...
type PracticeSummary = {
  summaryId: string;
  sessionId: string;
  sessionType: "guided_hydraulic_practice" | "hydraulic_independent_practice" | "mixed_mechanical_practice" | "guided_gear_practice" | "gear_independent_practice" | "gear_fluency" | "gear_assessment" | "guided_pulley_practice" | "pulley_independent_practice" | "pulley_assessment" | "guided_lever_practice" | "lever_independent_practice" | "lever_assessment" | "mixed_mechanical_assessment" | "guided_numerical_practice" | "numerical_independent_practice" | "numerical_assessment" | "guided_abstract_logical_practice" | "abstract_logical_independent_practice" | "abstract_logical_assessment" | "guided_verbal_practice" | "verbal_independent_practice" | "verbal_assessment";
  attempted: number;
  correct: number;
  accuracy: number;
...
function getDeviceClass(): "phone" | "tablet" | "desktop" {
...
type MechanicalCalibrationPilotRun = {
  runId: string;
  pilotVersion: string;
  startedAt: string;
  completedAt: string;
  responses: MechanicalCalibrationPilotResponse[];
};
...
function downloadMechanicalCalibrationPilotCsv() {
  ...
  anchor.href = url;
  anchor.download = `aptesta-mechanical-calibration-${new Date().toISOString().slice(0, 10)}.csv`;
...
type NumericalCalibrationPilotRun = {
  runId: string;
  pilotVersion: string;
  startedAt: string;
  completedAt: string;
  responses: NumericalCalibrationPilotResponse[];
};

const NUMERICAL_CALIBRATION_PILOT_STORAGE_KEY = "aptesta.numericalCalibrationPilot.v0_9";

function loadNumericalCalibrationPilotRuns(): NumericalCalibrationPilotRun[] {
  if (typeof window === "undefined") return [];
  try {
    const stored = JSON.parse(window.localStorage.getItem(NUMERICAL_CALIBRATION_PILOT_STORAGE_KEY) || "[]");
...
function saveNumericalCalibrationPilotRun(run: NumericalCalibrationPilotRun) {
  if (typeof window === "undefined") return;
  try {
    const stored = JSON.parse(window.localStorage.getItem(NUMERICAL_CALIBRATION_PILOT_STORAGE_KEY) || "[]");
    const existing = Array.isArray(stored) ? stored : [];
    window.localStorage.setItem(NUMERICAL_CALIBRATION_PILOT_STORAGE_KEY, JSON.stringify([...existing, run]));
  } catch {
    window.localStorage.setItem(NUMERICAL_CALIBRATION_PILOT_STORAGE_KEY, JSON.stringify([run]));
  }
}

function downloadNumericalCalibrationPilotCsv() {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  ...
  runs.forEach((run) => run.responses.forEach((response) => {
    ...
    rows.push([
      "APTESTA_NUM_CAL_EXPORT_V0_12", run.pilotVersion, run.runId, response.questionId, item?.blueprintId, item?.familyId, item?.archetype, item?.responseFormat ?? "mcq", item?.difficulty, item?.reasoningSteps, item?.targetTimeRangeSec.minSec, item?.targetTimeRangeSec.maxSec, response.enteredValue ?? response.selectedOptionId ?? "", response.correct, Math.round(response.selectionTimeMs), response.deviceClass, response.misconceptionTag, response.answeredAt,
    ]);
  }));
  const csv = rows.map((row) => row.map(csvCell).join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `aptesta-numerical-calibration-${new Date().toISOString().slice(0, 10)}.csv`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
...
type AbstractLogicalCalibrationPilotRun = {
  runId: string;
  pilotVersion: string;
  startedAt: string;
  completedAt: string;
  responses: AbstractLogicalCalibrationPilotResponse[];
};

const ABSTRACT_LOGICAL_CALIBRATION_PILOT_STORAGE_KEY = "aptesta.abstractLogicalCalibrationPilot.v0_12";

function loadAbstractLogicalCalibrationPilotRuns(): AbstractLogicalCalibrationPilotRun[] {
  if (typeof window === "undefined") return [];
  try {
    const stored = JSON.parse(window.localStorage.getItem(ABSTRACT_LOGICAL_CALIBRATION_PILOT_STORAGE_KEY) || "[]");
    return Array.isArray(stored) ? stored : [];
  } catch {
    return [];
  }
...
function saveAbstractLogicalCalibrationPilotRun(run: AbstractLogicalCalibrationPilotRun) {
  if (typeof window === "undefined") return;
  try {
    const stored = JSON.parse(window.localStorage.getItem(ABSTRACT_LOGICAL_CALIBRATION_PILOT_STORAGE_KEY) || "[]");
    const existing = Array.isArray(stored) ? stored : [];
    window.localStorage.setItem(ABSTRACT_LOGICAL_CALIBRATION_PILOT_STORAGE_KEY, JSON.stringify([...existing, run]));
  } catch {
    window.localStorage.setItem(ABSTRACT_LOGICAL_CALIBRATION_PILOT_STORAGE_KEY, JSON.stringify([run]));
  }
}

function downloadAbstractLogicalCalibrationPilotCsv() {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  const runs = loadAbstractLogicalCalibrationPilotRuns();
  const rows: (string | number | boolean | null | undefined)[][] = [[
    "export_version", "pilot_version", "run_id", "question_id", "blueprint_id", "family_id", "archetype", "difficulty", "reasoning_steps", "target_min_sec", "target_max_sec", "selected_response", "correct", "selection_time_ms", "device_class", "misconception_tag", "answered_at",
  ]];
  runs.forEach((run) => run.responses.forEach((response) => {
    const item = abstractLogicalCalibrationAllItems.find((candidate) => candidate.questionId === response.questionId);
    rows.push([
      "APTESTA_ABSTRACT_LOGICAL_CAL_EXPORT_V0_12", run.pilotVersion, run.runId, response.questionId, item?.blueprintId, item?.familyId, item?.archetype, item?.difficulty, item?.reasoningSteps, item?.targetTimeRangeSec.minSec, item?.targetTimeRangeSec.maxSec, response.selectedOptionId, response.correct, Math.round(response.selectionTimeMs), response.deviceClass, response.misconceptionTag, response.answeredAt,
    ]);
  }));
  const csv = rows.map((row) => row.map(csvCell).join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `aptesta-abstract-logical-calibration-${new Date().toISOString().slice(0, 10)}.csv`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
...
type VerbalCalibrationPilotResponse = {
  questionId: string;
...
type VerbalCalibrationPilotRun = {
  runId: string;
  pilotVersion: string;
  startedAt: string;
  completedAt: string;
  responses: VerbalCalibrationPilotResponse[];
};

const VERBAL_CALIBRATION_PILOT_STORAGE_KEY = "aptesta.verbalCalibrationPilot.v0_15";

function loadVerbalCalibrationPilotRuns(): VerbalCalibrationPilotRun[] {
  if (typeof window === "undefined") return [];
  try {
    const stored = JSON.parse(window.localStorage.getItem(VERBAL_CALIBRATION_PILOT_STORAGE_KEY) || "[]");
    return Array.isArray(stored) ? stored : [];
  } catch {
    return [];
  }
...
function saveVerbalCalibrationPilotRun(run: VerbalCalibrationPilotRun) {
  if (typeof window === "undefined") return;
  try {
    const stored = JSON.parse(window.localStorage.getItem(VERBAL_CALIBRATION_PILOT_STORAGE_KEY) || "[]");
    const existing = Array.isArray(stored) ? stored : [];
    window.localStorage.setItem(VERBAL_CALIBRATION_PILOT_STORAGE_KEY, JSON.stringify([...existing, run]));
  } catch {
    window.localStorage.setItem(VERBAL_CALIBRATION_PILOT_STORAGE_KEY, JSON.stringify([run]));
  }
}

function downloadVerbalCalibrationPilotCsv() {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  const runs = loadVerbalCalibrationPilotRuns();
  const rows: (string | number | boolean | null | undefined)[][] = [[
    "export_version", "pilot_version", "run_id", "question_id", "blueprint_id", "family_id", "archetype", "difficulty", "reasoning_steps", "target_min_sec", "target_max_sec", "selected_response", "correct", "selection_time_ms", "device_class", "misconception_tag", "answered_at",
  ]];
  runs.forEach((run) => run.responses.forEach((response) => {
    const item = verbalCalibrationAllItems.find((candidate) => candidate.questionId === response.questionId);
    rows.push([
      "APTESTA_VERBAL_CAL_EXPORT_V0_16", run.pilotVersion, run.runId, response.questionId, item?.blueprintId, item?.familyId, item?.archetype, item?.difficulty, item?.reasoningSteps, item?.targetTimeRangeSec.minSec, item?.targetTimeRangeSec.maxSec, response.selectedOptionId, response.correct, Math.round(response.selectionTimeMs), response.deviceClass, response.misconceptionTag, response.answeredAt,
    ]);
  }));
  const csv = rows.map((row) => row.map(csvCell).join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `aptesta-verbal-calibration-${new Date().toISOString().slice(0, 10)}.csv`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
...
type CalibrationReadinessRow = {
  ...
  targetMinSec: number;
  targetMaxSec: number;
  rapidRecognitionIntent: string;
  implementationStatus: string;
  qaStatus: string;
  linkedFamilyId: string;
  authoredVariantCount: number;
  authoredQuestionIds: string[];
  authoredFamilyIds: string[];
  authoredReasoningSteps: string[];
  authoredTargetRanges: string[];
  authoredLinked: boolean;
  familyLinkConsistent: boolean;
  coreMetadataComplete: boolean;
};

function getCalibrationAuthoredLinks(): CalibrationAuthoredLink[] {
  const gearDirectionLinks: CalibrationAuthoredLink[] = gearDirectionCalibrationBlueprints.map((item) => ({
    questionId: item.questionId,
    blueprintId: "MECHANICAL-01",
    familyId: item.familyId,
    archetype: item.archetype,
    reasoningSteps: item.reasoningSteps,
    targetMinSec: item.targetFluentTimeRangeSec.minSec,
    targetMaxSec: item.targetFluentTimeRangeSec.maxSec,
    difficulty: item.difficultyBand,
    responseFormat: "Fluency direction choice",
  }));

  const mechanicalLinks: CalibrationAuthoredLink[] = mechanicalCalibrationPilotItems.map((item) => ({
    questionId: item.questionId,
    blueprintId: item.blueprintId,
    familyId: item.familyId,
    archetype: item.archetype,
    reasoningSteps: item.reasoningSteps,
    targetMinSec: item.targetTimeRangeSec.minSec,
    targetMaxSec: item.targetTimeRangeSec.maxSec,
    difficulty: item.difficulty,
    responseFormat: "Diagram MCQ",
  }));

  const numericalLinks: CalibrationAuthoredLink[] = numericalCalibrationAllItems.map((item) => ({
    questionId: item.questionId,
    blueprintId: item.blueprintId,
    familyId: item.familyId,
    archetype: item.archetype,
    ...
    targetMaxSec: item.targetTimeRangeSec.maxSec,
    difficulty: item.difficulty,
    responseFormat: item.responseFormat ?? "mcq",
  }));

  const abstractLinks: CalibrationAuthoredLink[] = abstractLogicalCalibrationAllItems.map((item) => ({
    questionId: item.questionId,
    blueprintId: item.blueprintId,
    familyId: item.familyId,
    archetype: item.archetype,
    ...
    targetMaxSec: item.targetTimeRangeSec.maxSec,
    difficulty: item.difficulty,
    responseFormat: "Visual/text MCQ",
  }));

  const verbalLinks: CalibrationAuthoredLink[] = verbalCalibrationAllItems.map((item) => ({
    questionId: item.questionId,
    blueprintId: item.blueprintId,
    familyId: item.familyId,
    archetype: item.archetype,
    ...
    targetMaxSec: item.targetTimeRangeSec.maxSec,
    difficulty: item.difficulty,
    responseFormat: "Passage/text MCQ",
  }));

  return [...gearDirectionLinks, ...mechanicalLinks, ...numericalLinks, ...abstractLinks, ...verbalLinks];
}

function getCalibrationReadinessRows(): CalibrationReadinessRow[] {
  const authoredLinks = getCalibrationAuthoredLinks();
  return generalCalibrationBlueprints.map((blueprint) => {
    const linked = authoredLinks.filter((item) => item.blueprintId === blueprint.blueprintId);
    const familyIds = Array.from(new Set(linked.map((item) => item.familyId)));
    const linkedFamilyId = blueprint.linkedLiveFamilyId ?? "";
    const coreMetadataComplete = Boolean(
      blueprint.blueprintId && blueprint.domain && blueprint.subskill && blueprint.archetype && blueprint.itemBrief &&
      blueprint.responseFormat && blueprint.difficulty && blueprint.calibrationPurpose && blueprint.rapidRecognitionIntent &&
      blueprint.targetTimeRangeSec.minSec > 0 && blueprint.targetTimeRangeSec.maxSec >= blueprint.targetTimeRangeSec.minSec &&
      linkedFamilyId,
    );
    return {
      ...
      itemBrief: blueprint.itemBrief,
      responseFormat: blueprint.responseFormat,
      difficulty: blueprint.difficulty,
      targetMinSec: blueprint.targetTimeRangeSec.minSec,
      targetMaxSec: blueprint.targetTimeRangeSec.maxSec,
      rapidRecognitionIntent: blueprint.rapidRecognitionIntent,
      implementationStatus: blueprint.implementationStatus,
      qaStatus: blueprint.qaStatus,
      linkedFamilyId,
      authoredVariantCount: linked.length,
      authoredQuestionIds: linked.map((item) => item.questionId),
      authoredFamilyIds: familyIds,
      authoredReasoningSteps: Array.from(new Set(linked.map((item) => String(item.reasoningSteps)))),
      authoredTargetRanges: Array.from(new Set(linked.map((item) => `${item.targetMinSec}-${item.targetMaxSec}s`))),
      authoredLinked: linked.length > 0,
      familyLinkConsistent: linked.length > 0 && familyIds.length === 1 && familyIds[0] === linkedFamilyId,
      coreMetadataComplete,
    };
  });
}

function downloadCalibrationReadinessCsv() {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  const rows = getCalibrationReadinessRows();
  const output: (string | number | boolean | null | undefined)[][] = [[
    "export_version", "framework_version", "blueprint_id", "domain", "sequence", "subskill", "archetype", "item_brief", "response_format", "difficulty", "target_min_sec", "target_max_sec", "rapid_recognition_intent", "implementation_status", "qa_status", "linked_family_id", "authored_variant_count", "authored_question_ids", "authored_family_ids", "authored_reasoning_steps", "authored_target_ranges", "authored_linked", "family_link_consistent", "core_metadata_complete", "empirical_status", "profile_linkage_model",
  ]];
  rows.forEach((row) => output.push([
    "APTESTA_CALIBRATION_READINESS_EXPORT_V0_18",
    ...
    row.targetMinSec,
    row.targetMaxSec,
    row.rapidRecognitionIntent,
    row.implementationStatus,
    row.qaStatus,
    row.linkedFamilyId,
    row.authoredVariantCount,
    row.authoredQuestionIds.join(";"),
    row.authoredFamilyIds.join(";"),
    row.authoredReasoningSteps.join(";"),
    row.authoredTargetRanges.join(";"),
    row.authoredLinked,
    row.familyLinkConsistent,
    row.coreMetadataComplete,
    "not_empirically_calibrated",
    "provider_and_employer_profiles_kept_separate",
  ]));
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `aptesta-calibration-readiness-v0-18-${new Date().toISOString().slice(0, 10)}.csv`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
...
function hasCalibrationTiming(response: AssessmentResponse) {
  return response.responseTimeBasis === "selection_latency" && response.telemetryVersion === gearDirectionFluencyProfile.telemetryVersion;
}
...
const hydraulicIndependentPracticeQuestions: MvpQuestion[] = [
  makeHydraulicIndependentQuestion("HYD-IP-001", "pressure_transmission", "In an ideal sealed hydraulic system, the input piston creates a pressure of 6 N/cm². What pressure reaches the output piston?", ["6 N/cm²", "12 N/cm²", "3 N/cm²", "It depends on piston size"], "A", "In an ideal confined fluid, pressure is transmitted unchanged through the system."),
  makeHydraulicIndependentQuestion("HYD-IP-002", "piston_area_force", "The same pressure acts on two pistons. Piston B has three times the area of Piston A. Compared with Piston A, Piston B produces ideally:", ["Three times the force", "The same force", "One-third of the force", "Nine times the force"], "A", "At the same pressure, three times the area produces three times the force."),
  makeHydraulicIndependentQuestion("HYD-IP-003", "pressure_calculation", "A force of 48 N acts on a piston area of 6 cm². What pressure is created?", ["8 N/cm²", "42 N/cm²", "54 N/cm²", "288 N/cm²"], "A", "Pressure equals force divided by area: 48 ÷ 6 = 8 N/cm²."),
  makeHydraulicIndependentQuestion("HYD-IP-004", "force_calculation", "A hydraulic ram receives a pressure of 9 N/cm² across a piston area of 20 cm². What ideal force does it produce?", ["180 N", "29 N", "11 N", "450 N"], "A", "Force equals pressure multiplied by area: 9 × 20 = 180 N."),
  makeHydraulicIndependentQuestion("HYD-IP-005", "force_multiplication", "A car jack has a 4 cm² input piston and a 20 cm² lifting piston. If 50 N is applied to the input, what ideal lifting force is produced?", ["250 N", "50 N", "200 N", "1,000 N"], "A", "The output piston has five times the area, so it produces five times the force: 250 N."),
  makeHydraulicIndependentQuestion("HYD-IP-006", "movement_tradeoff", "A hydraulic system multiplies force by using a larger output piston. In an ideal system, what trade-off should you expect?", ["The output piston moves a shorter distance", "The output pressure disappears", "The output piston moves a longer distance", "The input force becomes zero"], "A", "Force is gained at the expense of movement distance; the larger output piston generally moves less far."),
  makeHydraulicIndependentQuestion("HYD-IP-007", "connected_pistons", "The left piston in a sealed two-piston system is pushed downward. What will the right piston usually do?", ["Move upward", "Move downward", "Remain fixed", "Rotate clockwise"], "A", "The input displaces fluid, which drives the other piston upward."),
  makeHydraulicIndependentQuestion("HYD-IP-008", "braking_application", "Why can a larger wheel-cylinder piston in a hydraulic brake produce more force than the master-cylinder piston?", ["The same pressure acts over a larger area", "Pressure increases as it travels", "The brake fluid becomes heavier", "The larger piston always moves farther"], "A", "The fluid transmits pressure; applying that pressure over a larger piston area produces greater force."),
  makeHydraulicIndependentQuestion("HYD-IP-009", "compare_arrangements", "Four hydraulic output pistons receive the same pressure. Which produces the greatest force?", ["The piston with the largest area", "The piston with the smallest area", "They all produce the same force", "The piston furthest from the input"], "A", "With pressure held constant, the largest area produces the greatest force."),
  makeHydraulicIndependentQuestion("HYD-IP-010", "integrated_reasoning", "An input piston has an area of 5 cm² and receives 40 N. The output piston has an area of 25 cm². What ideal output force is produced?", ["200 N", "40 N", "120 N", "1,000 N"], "A", "Input pressure is 40 ÷ 5 = 8 N/cm². The same pressure acts on 25 cm², so output force is 8 × 25 = 200 N."),
];
...
const gearFluencyFormAQuestions: MvpQuestion[] = [
  ...
  makeGearFluencyQuestion("GEAR-FL-004", "five_gear_direction", "Gear A turns clockwise through a train of five gears. Which way does Gear E turn?", ["Clockwise", "Anticlockwise", "It cannot turn", "It depends on gear size"], "A", "Four contacts give an even number of reversals, so Gear E turns the same way as Gear A."),
  makeGearFluencyQuestion("GEAR-FL-005", "six_gear_direction", "Gear A turns anticlockwise through a train of six gears. Which way does Gear F turn?", ["Clockwise", "Anticlockwise", "It cannot turn", "Direction cannot be known"], "A", "Five contacts give an odd number of reversals, so Gear F turns opposite to Gear A."),
  makeGearFluencyQuestion("GEAR-FL-006", "middle_driver_direction", "Gear C is the driver in a three-gear train and turns anticlockwise. What direction does the gear immediately to its left turn?", ["Clockwise", "Anticlockwise", "It does not turn", "Direction cannot be known"], "A", "A gear directly meshing with an anticlockwise driver turns clockwise."),
...
const pulleyIndependentPracticeQuestionPool: MvpQuestion[] = [
...
const pulleyIndependentCoreQuestionIds = new Set([
  "PULL-IP-001",
  "PULL-IP-002",
  "PULL-IP-003",
  "PULL-IP-004",
...
const pulleyIndependentPracticeQuestions: MvpQuestion[] = pulleyIndependentPracticeQuestionPool.filter((question) => pulleyIndependentCoreQuestionIds.has(question.questionId));
...
const leverIndependentPracticeQuestionPool: MvpQuestion[] = [
...
const leverIndependentCoreQuestionIds = new Set([
  "LEV-IP-001",
  "LEV-IP-002",
  "LEV-IP-003",
  "LEV-IP-004",
...
const leverIndependentPracticeQuestions: MvpQuestion[] = leverIndependentPracticeQuestionPool.filter((question) => leverIndependentCoreQuestionIds.has(question.questionId));
...
const mixedMechanicalPracticeQuestionPool: MvpQuestion[] = [
...
const mixedMechanicalPracticeCoreQuestionIds = new Set([
  "MIX-HYD-001",
  "MIX-HYD-002",
  "MIX-HYD-004",
  "MIX-GEAR-001",
...
const mixedMechanicalPracticeQuestions: MvpQuestion[] = mixedMechanicalPracticeQuestionPool.filter((question) => mixedMechanicalPracticeCoreQuestionIds.has(question.questionId));
...
const mixedMechanicalAssessmentQuestionPool: MvpQuestion[] = [
...
const mixedMechanicalAssessmentCoreQuestionIds = new Set([
  "MMA-HYD-001",
  "MMA-HYD-002",
  "MMA-HYD-004",
  "MMA-HYD-005",
...
const mixedMechanicalAssessmentQuestions: MvpQuestion[] = mixedMechanicalAssessmentQuestionPool.filter((question) => mixedMechanicalAssessmentCoreQuestionIds.has(question.questionId));
...
const guidedAbstractLogicalPracticeQuestions: MvpQuestion[] = [
  ...
  makeAbstractLogicalQuestion("ABS-G-011", "matrices_rules", "subtraction_rule", "What belongs in the missing cell?", ["▲", "▲▲", "▲▲▲", "○"], "A", "In each row, the third cell is what remains after the second count is removed from the first. Four triangles minus three leaves one triangle.", "Describe the relationship between the cells before choosing an answer.", {"kind": "matrix", "rows": [["▲▲▲", "▲▲", "▲"], ["▲▲▲▲", "▲▲▲", "?"]]}),
  makeAbstractLogicalQuestion("ABS-G-012", "classification_relationships", "shared_property", "Which option belongs with the group shown?", ["□", "○", "△", "●"], "A", "The group contains four-sided shapes. The square shares that defining property.", "Find the property shared by the group before looking at the options.", {"kind": "set", "rows": [["▭", "◇", "▱", "?"]]}),
  makeAbstractLogicalQuestion("ABS-G-013", "deductive_reasoning", "conditional", "If the alarm sounds, the warning light turns on. The alarm sounds. What must follow?", ["The warning light turns on", "The warning light stays off", "The alarm is faulty", "Nothing can be concluded"], "A", "The stated condition applies directly: alarm sounds → warning light on.", "Translate the statement into a simple if → then rule.", undefined),
].map((question) => ({ ...question, sessionType: "guided_abstract_logical_practice" as const }));

const abstractLogicalIndependentPracticeQuestionBank: MvpQuestion[] = [
...
// Core v1 set: 12 questions, balanced 3 per subcompetency.
// The full 25-question bank is retained above for future alternate/repeat sets.
  ...
  sections: [
    {
      ...
      title: "One mesh, one reversal",
      body: "Start with the simplest gear relationship. When two gears mesh directly, the teeth push against each other. If the driver turns clockwise, the driven gear must turn anticlockwise.",
      keyPoint: "Every direct gear mesh reverses direction.",
      miniCheck: makeMiniCheck("GEAR-FUND-MC-001", "Gear A turns clockwise and meshes directly with Gear B. Which way does Gear B turn?", ["Anticlockwise", "Clockwise", "It depends on gear size", "It cannot turn"], "A", "One direct mesh means one reversal, so Gear B turns anticlockwise."),
    },
    {
      sectionId: "gear-fund-002",
      title: "Find the driver first",
      body: "Before counting anything, identify the gear whose motion is given. That is the driver. Its direction is your starting point. From there, trace only the gears that are actually connected to the drive path.",
      keyPoint: "Start with the driver. Do not guess from the largest or most obvious gear.",
      miniCheck: makeMiniCheck("GEAR-FUND-MC-002", "What is the best first step in a gear-direction question?", ["Find the driver gear", "Find the largest gear", "Count every gear in the picture", "Assume the last gear turns clockwise"], "A", "The driver gives you the starting direction for the gear train."),
    },
    {
      sectionId: "gear-fund-003",
      title: "Two reversals return to the start",
      body: "Now add a third gear. A turns clockwise. B reverses to anticlockwise. C reverses again to clockwise. The last gear therefore turns the same way as the first.",
      keyPoint: "A → B is one reversal. B → C is a second reversal. Two reversals restore the original direction.",
      miniCheck: makeMiniCheck("GEAR-FUND-MC-003", "Three gears are in a row. Gear A turns clockwise. Which way does Gear C turn?", ["Clockwise", "Anticlockwise", "It cannot turn", "Direction cannot be known"], "A", "There are two meshes, so direction reverses twice. Gear C turns clockwise."),
    },
    {
    ...
    },
    {
      ...
      title: "Do not let size distract you",
      body: "Large and small gears can turn at different speeds, but every direct mesh still reverses direction. If the question asks only about direction, gear size is often a distractor.",
      keyPoint: "Size can change speed. It does not change the one-mesh-one-reversal rule.",
      miniCheck: makeMiniCheck("GEAR-FUND-MC-005", "A very small clockwise gear drives a much larger gear. Which way does the larger gear turn?", ["Anticlockwise", "Clockwise", "The direction depends on the number of teeth", "It does not turn"], "A", "The gears mesh directly, so the larger gear turns anticlockwise. Size does not change the direction rule."),
    },
    {
...
function createAssessmentResponse(
  ...
  responseTimeMs: number,
  notSureSelected: boolean,
  telemetry: Partial<Pick<AssessmentResponse, "responseTimeBasis" | "firstActionTimeMs" | "telemetryVersion" | "deviceClass">> = {},
): AssessmentResponse {
  return { responseId: id("response"), sessionId, questionId: question.questionId, selectedOptionId, correct: selectedOptionId === question.correctOptionId, responseTimeMs, notSureSelected, answeredAt: now(), ...telemetry };
}
...
function completePulleyAssessment(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  const previousRecommendation = getCurrentRecommendation(journey);
  const updatedRecommendations = journey.recommendations.map((rec) => rec.recommendationId === previousRecommendation?.recommendationId ? { ...rec, status: "completed" as const } : rec);
  let recommendationType: Recommendation["recommendationType"] = "start_lever_fundamentals";
  let title = "Next: Lever Fundamentals";
  let summaryText = "Your pulley pathway is complete. Build the next mechanical reasoning skill before mixed practice.";
  let actionLabel = "Start Lever Fundamentals";
  let currentFocus = "Lever reasoning";
  let interpretation = "The Pulley Check is strong enough to close the pulley pathway and move to Lever Fundamentals.";
...
function completeVerbalAssessment(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  const previousRecommendation = getCurrentRecommendation(journey);
  const updatedRecommendations = journey.recommendations.map((rec) => rec.recommendationId === previousRecommendation?.recommendationId ? { ...rec, status: "completed" as const } : rec);
  const priorCoreChecksPresent = ["mixed_mechanical_assessment", "numerical_assessment", "abstract_logical_assessment"].every((sessionType) =>
    journey.practiceSummaries.some((item) => item.sessionType === sessionType)
  );
  const coreJourneyComplete = balancedStrong && priorCoreChecksPresent;
  let recommendationType: Recommendation["recommendationType"] = "review_core_progress";
  let title = coreJourneyComplete ? "Core aptitude pathways complete" : "Review your core preparation";
  let summaryText = coreJourneyComplete
    ? "You have completed the core Vivalsa pathway across mechanical, numerical, abstract & logical, and verbal reasoning. Review the evidence collected across all four domains and choose any area you want to revisit."
    : "Your Verbal Check is strong. Review your preparation plan to see which other core domain checks still need completion or reinforcement.";
  let actionLabel = "Review core progress";
  let interpretation = coreJourneyComplete
    ? "This is a completion recommendation. No single verbal area was weak enough to justify targeted remediation, and evidence has now been collected from all four core domain checks."
    : "This is a progression recommendation. Verbal reasoning is strong, but the complete four-domain evidence set is not yet present.";
  let recommendationKind: "progression" | "weakness" = "progression";
  ...
  const recommendation: Recommendation = { recommendationId: id("rec"), recommendationType, title, summary: summaryText, actionLabel, confidence: "moderate", whyExplanationId: why.whyExplanationId, status: "active", createdAt: now() };
  const readiness: ReadinessSnapshot = { readinessSnapshotId: id("readiness"), state: summary.accuracy >= 0.6 ? "developing_evidence" : "early_evidence", label: coreJourneyComplete ? "Developing evidence — core aptitude pathways completed" : balancedStrong ? "Developing evidence — verbal pathway completed" : "Developing evidence — verbal pathway continuing", explanation: interpretation, confidence: "moderate", createdAt: now() };
  ...
  const recentMilestoneIds = [...(journey.dashboardState?.recentMilestoneIds ?? []), ...newMilestones.map((m) => m.milestoneId)].slice(-7);
  return { ...journey, sessions: journey.sessions.map((s) => s.sessionId === sessionId ? completedSession : s), practiceSummaries: [...journey.practiceSummaries, summary], debriefs: [...journey.debriefs, debrief], competencyEvidence: [...journey.competencyEvidence, ...evidence], recommendations: [...updatedRecommendations, recommendation], whyExplanations: [...journey.whyExplanations, why], readinessSnapshots: [...journey.readinessSnapshots, readiness], milestones: [...journey.milestones, ...newMilestones], dashboardState: { ...(journey.dashboardState ?? { dashboardStateId: id("dash"), recentMilestoneIds: [], saveStatus: "local_only" as const, updatedAt: now() }), currentRecommendationId: recommendation.recommendationId, currentFocusLabel: coreJourneyComplete ? "Core aptitude pathways complete" : balancedStrong ? "Core preparation review" : "Verbal comprehension", readinessSnapshotId: readiness.readinessSnapshotId, recentMilestoneIds, baselineSummary: journey.dashboardState?.baselineSummary, saveStatus: "local_only", updatedAt: now() }, updatedAt: now() };
}
...
function completeGuidedHydraulicPractice(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  const strongResult = summary.accuracy >= 0.8;
  const developingResult = summary.accuracy >= 0.6 && summary.accuracy < 0.8;

  // Guided Practice completion always unlocks Independent Practice.
  // Review remains available as an optional supporting action rather than a gate.
  const recommendationType: Recommendation["recommendationType"] = "begin_hydraulic_independent_practice";
  const title = "Continue to Independent Practice";
  const actionLabel = "Continue to Independent Practice";
  ...
  if (strongResult) {
    summaryText = "Apply the hydraulic method across an independent set without guided prompts.";
    readinessLabel = "Developing evidence";
    readinessExplanation = "Guided performance is encouraging, but Independent Practice is needed to confirm transfer without support.";
    constraintStatus = "improving";
    observation = "You completed Guided Hydraulic Practice with a strong result.";
    interpretation = "The foundation concepts appear to be transferring with guidance. The next useful evidence is independent performance.";
    recommendationText = "Continue to Independent Hydraulic Practice. Review remains available if you would like to reinforce the fundamentals first.";
    confidenceText = "Moderate. The guided result is encouraging, but independent evidence is still required.";
    debriefSummary = "Your guided hydraulic reasoning showed a strong response.";
    debriefInterpretation = "Independent Practice is now the best next evidence check. Reviewing the fundamentals remains optional.";
  } else if (developingResult || improvement >= 0.2) {
    summaryText = "Test how well the hydraulic method transfers without guidance. A fundamentals review remains available if useful.";
    readinessLabel = "Early developing evidence";
    readinessExplanation = "The guided result shows some progress, but Independent Practice is needed to establish how consistent the method is without prompts.";
    constraintStatus = "improving";
    observation = "Your guided hydraulic reasoning shows some developing evidence.";
    interpretation = "The result is not yet stable, but Independent Practice will reveal where support is still needed.";
    recommendationText = "Continue to Independent Hydraulic Practice. You may review Hydraulic Fundamentals first if you would prefer more reinforcement.";
    confidenceText = "Moderate. The next independent set will provide more useful evidence than repeating the same guided step.";
    debriefSummary = "Your guided hydraulic reasoning is showing developing evidence.";
    debriefInterpretation = "Continue to Independent Practice to test transfer. Review Hydraulic Fundamentals remains an optional supporting step.";
  } else {
    summaryText = "Continue into Independent Practice to identify exactly which hydraulic relationships still need reinforcement.";
    readinessLabel = "Early evidence — improvement not yet confirmed";
    readinessExplanation = "Current guided evidence does not yet show a clear improvement signal. Independent Practice will provide a more precise picture of what still needs work.";
    constraintStatus = "active";
    observation = "The guided result does not yet show a clear improvement signal.";
    interpretation = "Some fundamentals may still need reinforcement, but Independent Practice is the next useful evidence check rather than a locked step.";
    recommendationText = "Continue to Independent Hydraulic Practice. Review Hydraulic Fundamentals is also available as an optional secondary action.";
    confidenceText = "Moderate. Independent performance will clarify whether the difficulty is broad or limited to particular hydraulic relationships.";
    debriefSummary = "Current evidence does not yet show clear improvement in hydraulic-force reasoning.";
    debriefInterpretation = "Independent Practice is still unlocked and will help identify what needs reinforcement. You may review Hydraulic Fundamentals first if preferred.";
  }
  ...
  const why: WhyExplanation = {
    whyExplanationId: id("why"),
    title: "Why Independent Hydraulic Practice is recommended",
    observation,
  ...
  const previousRecommendation = getCurrentRecommendation(journey);
  const updatedRecommendations = journey.recommendations.map((rec) =>
    rec.recommendationId === previousRecommendation?.recommendationId
      ? { ...rec, status: "completed" as const }
      : rec
  );
  const updatedConstraints = journey.constraints.map((constraint) =>
    constraint.subcompetency === "hydraulics"
      ? { ...constraint, status: constraintStatus, updatedAt: now() }
      : constraint
  );
  const readiness: ReadinessSnapshot = {
    readinessSnapshotId: id("readiness"),
    state: strongResult || developingResult ? "developing_evidence" : "early_evidence",
    label: readinessLabel,
    explanation: readinessExplanation,
    confidence: "high",
    createdAt: now(),
  };
  const guidedMilestone: Milestone = {
    milestoneId: id("milestone"),
    type: "guided_hydraulic_practice_completed",
    label: "Guided Hydraulic Practice completed",
    createdAt: now(),
  };
  const improvementMilestone: Milestone | null = strongResult || developingResult
    ? {
        milestoneId: id("milestone"),
        type: "first_improvement_signal",
        label: strongResult ? "First improvement signal detected" : "Early improvement signal detected",
        createdAt: now(),
      }
    : null;
  const newMilestones = improvementMilestone
    ? [guidedMilestone, improvementMilestone]
    : [guidedMilestone];
  const debrief: Debrief = {
    ...
    comparison: evidenceText,
    interpretation: debriefInterpretation,
    recommendationId: recommendation.recommendationId,
    confidence: "moderate",
    whyExplanationId: why.whyExplanationId,
    createdAt: now(),
  };
  const recentMilestoneIds = [
    ...(journey.dashboardState?.recentMilestoneIds ?? []),
    ...newMilestones.map((milestone) => milestone.milestoneId),
  ].slice(-6);

  return {
    ...journey,
    sessions: journey.sessions.map((item) =>
      item.sessionId === sessionId ? completedSession : item
    ),
    ...
    dashboardState: {
      ...(journey.dashboardState ?? {
        dashboardStateId: id("dash"),
        recentMilestoneIds: [],
        saveStatus: "local_only" as const,
        updatedAt: now(),
      }),
      currentRecommendationId: recommendation.recommendationId,
      currentFocusLabel: "Independent hydraulic reasoning",
      readinessSnapshotId: readiness.readinessSnapshotId,
      recentMilestoneIds,
...
function completeHydraulicIndependentPractice(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  const recommendationType: Recommendation["recommendationType"] = isProgression ? "start_gear_fundamentals" : needsMorePractice ? "continue_hydraulic_independent_practice" : "review_hydraulic_fundamentals";
  const title = isProgression ? "Next: Gear Direction" : needsMorePractice ? "Practise Hydraulic Reasoning once more" : "Review Hydraulic Fundamentals";
  const recommendationSummary = isProgression
    ? "Your hydraulic reasoning is now working independently. Gear Direction is the next focused mechanical skill: it adds a new reasoning pattern before topics are mixed together."
    : needsMorePractice
  ...
  const why: WhyExplanation = {
    whyExplanationId: id("why"),
    title: isProgression ? "Why Gear Direction is recommended next" : "Why this strengthening step is recommended",
  ...
  return {
    ...
    readinessSnapshots: [...journey.readinessSnapshots, readiness],
    milestones: [...journey.milestones, milestone],
    dashboardState: { ...(journey.dashboardState ?? { dashboardStateId: id("dash"), recentMilestoneIds: [], saveStatus: "local_only" as const, updatedAt: now() }), currentRecommendationId: recommendation.recommendationId, currentFocusLabel: isProgression ? "Gear direction" : "Hydraulic reasoning", readinessSnapshotId: readiness.readinessSnapshotId, recentMilestoneIds, baselineSummary: journey.dashboardState?.baselineSummary, saveStatus: journey.dashboardState?.saveStatus ?? "local_only", updatedAt: now() },
    updatedAt: now(),
  };
}
...
function completeGearAssessment(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  let recommendationType: Recommendation["recommendationType"] = "start_pulley_fundamentals";
  let recommendationTitle = "Next: Pulley Fundamentals";
  let recommendationSummary = "Your gear pathway is complete. Build the next mechanical reasoning skill before mixed practice.";
  let actionLabel = "Start Pulley Fundamentals";
  let currentFocus = "Pulley reasoning";
  let readinessLabel = "Developing evidence — gear pathway completed";
  let readinessExplanation = "Your Gear Check was strong enough to close the gear pathway and move to the next mechanical topic.";
  ...
  let interpretation = "This is good evidence that the core gear pathway has done its job. The next useful step is Pulley Fundamentals before the mechanical topics are mixed together.";
  let whyObservation = "The Gear Check showed consistent independent gear reasoning.";
  let whyInterpretation = "The result is strong enough to close the isolated gear pathway and move to Pulley Fundamentals.";
...
function completeMixedMechanicalPractice(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  const readyForAssessment = summary.accuracy >= 0.8 && breakdown.every((item) => item.attempted > 0 && item.accuracy >= 2 / 3);
  ...
  if (readyForAssessment) {
    recommendationType = "begin_mixed_mechanical_assessment";
    title = "Take the Mixed Mechanical Assessment";
    summaryText = "The four mechanical methods remained available when they were interleaved. The next useful step is a no-feedback integrated check.";
    actionLabel = "Start mixed assessment";
    currentFocus = "Integrated mechanical reasoning";
    interpretation = "This mixed practice was strong across all four areas. The next step is assessment-style evidence rather than more isolated mechanical teaching.";
  } else {
    currentFocus = `${weakestLabel} reasoning`;
    const weak = weakest?.accuracy ?? 0;
    if (weakest?.concept === "hydraulics") {
      recommendationType = weak <= 1 / 3 ? "begin_guided_hydraulic_practice" : "continue_hydraulic_independent_practice";
      title = weak <= 1 / 3 ? "Target Guided Hydraulic Practice" : "Target Independent Hydraulic Practice";
      actionLabel = weak <= 1 / 3 ? "Start guided practice" : "Start independent practice";
    } else if (weakest?.concept === "gears") {
      recommendationType = weak <= 1 / 3 ? "continue_guided_gear_practice" : "continue_gear_independent_practice";
      title = weak <= 1 / 3 ? "Target Guided Gear Practice" : "Target Independent Gear Practice";
      actionLabel = weak <= 1 / 3 ? "Start guided practice" : "Start independent practice";
    } else if (weakest?.concept === "pulleys") {
      recommendationType = weak <= 1 / 3 ? "continue_guided_pulley_practice" : "continue_pulley_independent_practice";
      title = weak <= 1 / 3 ? "Target Guided Pulley Practice" : "Target Independent Pulley Practice";
      actionLabel = weak <= 1 / 3 ? "Start guided practice" : "Start independent practice";
    } else {
      recommendationType = weak <= 1 / 3 ? "continue_guided_lever_practice" : "continue_lever_independent_practice";
      title = weak <= 1 / 3 ? "Target Guided Lever Practice" : "Target Independent Lever Practice";
      actionLabel = weak <= 1 / 3 ? "Start guided practice" : "Start independent practice";
    }
    summaryText = `${weakestLabel} was the least stable area once the four mechanical methods were mixed. Use a short targeted set before another integrated attempt.`;
    interpretation = `This is a strengthening recommendation. ${weakestLabel} was the clearest area to strengthen when the learner had to choose between mechanical methods.`;
  }

  const evidenceText = breakdown.map((item) => `${labels[item.concept] ?? item.concept}: ${item.correct}/${item.attempted}`).join(" · ");
  const why: WhyExplanation = {
    whyExplanationId: id("why"),
    title: `Why ${title} is recommended`,
    observation: readyForAssessment ? "Mixed Mechanical Practice was strong across all four mechanical areas." : `${weakestLabel} was the lowest area in Mixed Mechanical Practice.`,
    evidence: `Overall: ${summary.correct}/${summary.attempted}. ${evidenceText}.`,
    interpretation,
    recommendation: `${title} is recommended next.`,
    confidence: "Moderate. This is based on one 12-question mixed practice set and is intended to choose the next preparation step.",
    createdAt: now(),
  };
  ...
  const previousRecommendation = getCurrentRecommendation(journey);
  const updatedRecommendations = journey.recommendations.map((rec) =>
    rec.recommendationId === previousRecommendation?.recommendationId ? { ...rec, status: "completed" as const } : rec
  );

  const readiness: ReadinessSnapshot = {
    readinessSnapshotId: id("readiness"),
    state: "developing_evidence",
    label: readyForAssessment ? "Developing evidence — ready for integrated mechanical check" : `Developing evidence — ${weakestLabel.toLowerCase()} needs consolidation`,
  ...
  const completedMilestone: Milestone = {
    milestoneId: id("milestone"),
    type: "mixed_mechanical_practice_completed",
    label: "Mixed Mechanical Practice completed",
    createdAt: now(),
  };
  ...
  const debrief: Debrief = {
    debriefId: id("debrief"),
    sessionId,
    title: "Mixed mechanical practice complete",
    summary: `${summary.correct} of ${summary.attempted} correct.`,
    comparison: evidenceText,
    interpretation,
    recommendationId: recommendation.recommendationId,
    confidence: "moderate",
  ...
  const recentMilestoneIds = [...(journey.dashboardState?.recentMilestoneIds ?? []), completedMilestone.milestoneId].slice(-7);
  ...
  return {
    ...
    whyExplanations: [...journey.whyExplanations, why],
    readinessSnapshots: [...journey.readinessSnapshots, readiness],
    milestones: [...journey.milestones, completedMilestone],
    dashboardState: {
      ...(journey.dashboardState ?? { dashboardStateId: id("dash"), recentMilestoneIds: [], saveStatus: "local_only" as const, updatedAt: now() }),
      currentRecommendationId: recommendation.recommendationId,
      currentFocusLabel: currentFocus,
...
function completeMixedMechanicalAssessment(journey: MvpGuestJourney, sessionId: string): MvpGuestJourney {
  ...
  const weakest = [...summary.conceptBreakdown].sort((a, b) => a.accuracy - b.accuracy)[0];
  const integratedStrong = summary.accuracy >= 0.8 && summary.conceptBreakdown.every((item) => item.accuracy >= 3 / 4);
  const labels: Record<string, string> = { hydraulics: "Hydraulics", gears: "Gears", pulleys: "Pulleys", levers: "Levers" };
  const weakestLabel = labels[weakest?.concept ?? "gears"] ?? "Mechanical reasoning";
  ...
  const why: WhyExplanation = {
    ...
    interpretation,
    recommendation: `${title} is recommended next.`,
    confidence: "Moderate. This is based on one 16-question mixed assessment and should guide the next preparation step rather than define overall aptitude.",
    createdAt: now(),
  };
  const recommendation: Recommendation = { recommendationId: id("rec"), recommendationType, title, summary: summaryText, actionLabel, confidence: "moderate", whyExplanationId: why.whyExplanationId, status: "active", createdAt: now() };
  const readiness: ReadinessSnapshot = {
...
function getResumeState(journey: MvpGuestJourney): ResumeState {
  ...
  if (incompleteSession) {
    ...
    const screenBySessionType: Record<AssessmentSession["sessionType"], AppScreen> = {
      ...
      gear_fluency: "gear-fluency-question",
...
function DashboardScreen({
  ...
  onReviewCoreProgress,
  onLoadTestScenario,
  onOpenFluencyAnalytics,
  onOpenCalibrationFramework,
  onOpenCalibrationReadiness,
  onOpenMechanicalCalibrationPilot,
  onOpenNumericalCalibrationPilot,
  onOpenAbstractLogicalCalibrationPilot,
  onOpenVerbalCalibrationPilot,
  onRunGearFluency,
}: {
  journey: MvpGuestJourney;
  onWhy: () => void;
  ...
  onReviewCoreProgress: () => void;
  onLoadTestScenario: (scenario: TestScenario) => void;
  onOpenFluencyAnalytics: () => void;
  onOpenCalibrationFramework: () => void;
  onOpenCalibrationReadiness: () => void;
  onOpenMechanicalCalibrationPilot: () => void;
  onOpenNumericalCalibrationPilot: () => void;
  onOpenAbstractLogicalCalibrationPilot: () => void;
  onOpenVerbalCalibrationPilot: () => void;
  onRunGearFluency: () => void;
}) {
  const rec = getCurrentRecommendation(journey);
  ...
  const canReviewCoreProgress = rec?.recommendationType === "review_core_progress";
  ...
  const recommendationAction = canStartHydraulics ? <PrimaryButton onClick={onStartHydraulics}>Start fundamentals</PrimaryButton>
    ...
    : canReviewCoreProgress ? <PrimaryButton onClick={onReviewCoreProgress}>Review core progress</PrimaryButton>
  ...
  return <Shell right={timing}><section className="mx-auto max-w-6xl px-6 py-10 sm:px-8 sm:py-12">
    ...
    {SHOW_TEST_SCENARIOS && <details className="mt-6 rounded-2xl border border-white/5 bg-[#151A21] p-5"><summary className="cursor-pointer text-sm font-medium text-[#8D98A6]">Tester tools</summary><div className="mt-5"><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onRunGearFluency}>Open Gear Direction Fluency</SecondaryButton><SecondaryButton onClick={onOpenMechanicalCalibrationPilot}>Open mechanical calibration pilot</SecondaryButton><SecondaryButton onClick={onOpenNumericalCalibrationPilot}>Open numerical calibration pilot</SecondaryButton><SecondaryButton onClick={onOpenAbstractLogicalCalibrationPilot}>Open abstract/logical calibration pilot</SecondaryButton><PrimaryButton onClick={onOpenVerbalCalibrationPilot}>Open verbal calibration pilot</PrimaryButton><SecondaryButton onClick={onOpenFluencyAnalytics}>Open fluency analytics</SecondaryButton><SecondaryButton onClick={onOpenCalibrationFramework}>Open calibration framework</SecondaryButton><PrimaryButton onClick={onOpenCalibrationReadiness}>Open calibration readiness</PrimaryButton><SecondaryButton onClick={onReset}>Reset local journey</SecondaryButton></div><TestScenarioPanel onLoad={onLoadTestScenario} /></div></details>}
  </section></Shell>;
}
...
function GearPairDirectionDiagram() {
  const teeth = (cx: number, cy: number, r: number, count: number, keyPrefix: string) =>
    Array.from({ length: count }).map((_, index) => {
      const angle = (index * 360 / count) * Math.PI / 180;
      return <line key={`${keyPrefix}-${index}`} x1={cx + Math.cos(angle) * (r + 3)} y1={cy + Math.sin(angle) * (r + 3)} x2={cx + Math.cos(angle) * (r + 17)} y2={cy + Math.sin(angle) * (r + 17)} stroke="rgba(255,255,255,.25)" strokeWidth="6" strokeLinecap="round" />;
    });

  return (
    <div className="mt-7 overflow-hidden rounded-3xl border border-white/10 bg-[#111418] px-3 py-5 sm:p-7">
      <svg viewBox="0 0 620 260" role="img" aria-label="Two meshed gears rotating in opposite directions" className="h-auto w-full">
        <defs><marker id="pairArrow" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto"><path d="M0 0L10 5L0 10z" fill="#5ED3F3" /></marker></defs>
        <circle cx="210" cy="125" r="70" fill="#171C23" stroke="rgba(255,255,255,.22)" strokeWidth="4" />
        <circle cx="395" cy="125" r="70" fill="#171C23" stroke="rgba(255,255,255,.22)" strokeWidth="4" />
        {teeth(210,125,70,12,"a")}{teeth(395,125,70,12,"b")}
        <text x="210" y="133" textAnchor="middle" fill="#F4F6F8" fontSize="28" fontWeight="700">A</text>
        <text x="395" y="133" textAnchor="middle" fill="#F4F6F8" fontSize="28" fontWeight="700">B</text>
        <path d="M155 70 C115 110 115 160 155 200" fill="none" stroke="#5ED3F3" strokeWidth="7" markerEnd="url(#pairArrow)" />
        <path d="M450 200 C490 160 490 110 450 70" fill="none" stroke="#5ED3F3" strokeWidth="7" markerEnd="url(#pairArrow)" />
        <text x="210" y="242" textAnchor="middle" fill="#D9F8FF" fontSize="23" fontWeight="700">CLOCKWISE</text>
        <text x="395" y="242" textAnchor="middle" fill="#D9F8FF" fontSize="23" fontWeight="700">ANTICLOCKWISE</text>
      </svg>
    </div>
  );
}

function GearMeshCountingDiagram() {
  return (
    <div className="mt-7 rounded-3xl border border-white/10 bg-[#111418] p-5 sm:p-7">
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/5 bg-[#171C23] p-4">
          <p className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Odd meshes</p>
          <p className="mt-2 text-lg font-semibold text-[#F4F6F8]">1 · 3 · 5</p>
          <p className="mt-2 text-sm text-[#C8D2DD]">Target turns <span className="font-semibold text-[#D9F8FF]">opposite</span> to the driver.</p>
        </div>
        <div className="rounded-2xl border border-white/5 bg-[#171C23] p-4">
          <p className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Even meshes</p>
          <p className="mt-2 text-lg font-semibold text-[#F4F6F8]">2 · 4 · 6</p>
          <p className="mt-2 text-sm text-[#C8D2DD]">Target turns the <span className="font-semibold text-[#D9F8FF]">same way</span> as the driver.</p>
        </div>
      </div>
      <p className="mt-4 text-center text-base font-semibold text-[#5ED3F3]">COUNT THE MESHES — NOT THE GEARS</p>
    </div>
  );
}
...
function GearFundamentalsScreen({ journey, onSaveJourney, onComplete }: { journey: MvpGuestJourney; onSaveJourney: (journey: MvpGuestJourney) => void; onComplete: () => void }) {
  ...
  function goNext() {
    ...
    persistProgress({ ...progress, currentSectionIndex: Math.min(progress.currentSectionIndex + 1, gearFundamentalsModule.sections.length - 1), updatedAt: now() });
  }
    ...
    <Shell>
      <section className="mx-auto max-w-5xl px-5 pb-24 pt-7 sm:px-8 sm:pb-12 sm:pt-12">
        <div className="mb-5 h-1.5 overflow-hidden rounded-full bg-white/5">
          <div className="h-full rounded-full bg-[#5ED3F3]/70 transition-all" style={{ width: `${((progress.currentSectionIndex + 1) / gearFundamentalsModule.sections.length) * 100}%` }} />
        </div>

        <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Gear Direction</p>
            <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">{gearFundamentalsModule.title}</h1>
            <p className="mt-2 text-sm text-[#9AA3B2] sm:text-base">{gearFundamentalsModule.subtitle}</p>
          </div>
          <Badge>Section {progress.currentSectionIndex + 1} of {gearFundamentalsModule.sections.length}</Badge>
        </div>
        ...
        <div ref={sectionTopRef} className="scroll-mt-4">
          <Card className="p-5 sm:p-8">
            ...
            {section.sectionId === "gear-fund-001" && <GearPairDirectionDiagram />}
            {section.sectionId === "gear-fund-003" && <GearTrainDiagram />}
            ...
            {section.keyPoint && (
              <div className="mt-7 rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/10 p-5">
                <div className="text-xs uppercase tracking-[0.18em] text-[#6E7A88]">{section.sectionId === "gear-fund-008" ? "Pocket principle" : "Key idea"}</div>
...
function GuidedHydraulicPracticeDebriefScreen({ journey, onWhy, onNext, onReview, onDashboard }: { journey: MvpGuestJourney; onWhy: () => void; onNext: () => void; onReview: () => void; onDashboard: () => void }) {
  const debrief = getLatestDebrief(journey);
  const rec = getCurrentRecommendation(journey);

  return (
    <Shell>
      <section className="mx-auto flex min-h-[82vh] max-w-4xl items-center px-8 py-16">
        <Card>
          <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Practice debrief</p>
          <h1 className="mt-6 text-4xl font-semibold leading-tight">
            {debrief?.title ?? "Guided practice complete"}
          </h1>
          <p className="mt-6 text-lg leading-relaxed text-[#9AA3B2]">{debrief?.summary}</p>

          <div className="mt-8 grid gap-5">
            <div className="rounded-2xl border border-white/5 bg-[#111418] p-6">
              <div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Comparison</div>
              <p className="mt-3 text-[#C8D2DD]">{debrief?.comparison}</p>
            </div>

            <div className="rounded-2xl border border-white/5 bg-[#111418] p-6">
              <div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Interpretation</div>
              <p className="mt-3 text-[#C8D2DD]">{debrief?.interpretation}</p>
            </div>

            <div className="rounded-2xl border border-white/5 bg-[#111418] p-6">
              <div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Recommended next step</div>
              <h2 className="mt-3 text-2xl font-semibold">{rec?.title ?? "Continue to Independent Practice"}</h2>
              <p className="mt-3 text-[#AAB4C0]">{rec?.summary}</p>
              <div className="mt-5"><Badge>Moderate confidence</Badge></div>
            </div>
          </div>

          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <PrimaryButton onClick={onNext}>Continue to Independent Practice</PrimaryButton>
            <SecondaryButton onClick={onReview}>Review Hydraulic Fundamentals</SecondaryButton>
            <SecondaryButton onClick={onWhy}>Why this next step?</SecondaryButton>
            <SecondaryButton onClick={onDashboard}>View dashboard</SecondaryButton>
          </div>
        </Card>
      </section>
    </Shell>
...
function HydraulicIndependentPracticeIntroScreen({ onStart }: { onStart: () => void }) {
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-3xl items-center px-6 py-12 sm:px-8 sm:py-16"><Card className="text-center"><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Try it independently</p><h1 className="mt-5 text-4xl font-semibold">Independent Hydraulic Practice</h1><p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-[#9AA3B2]">Apply the hydraulic method without prompts across ten short, test-style questions. The contexts change, but the underlying relationships stay the same.</p><div className="mx-auto mt-8 grid max-w-xl gap-3 sm:grid-cols-3">{["10 questions", "Immediate feedback", "No guided prompts"].map((item) => <div key={item} className="rounded-xl border border-white/5 bg-[#111418] p-4 text-sm text-[#AAB4C0]">{item}</div>)}</div><p className="mx-auto mt-6 max-w-xl text-sm leading-relaxed text-[#8D98A6]">Work accurately first. Speed will come as the pattern becomes familiar.</p><div className="mt-9"><PrimaryButton onClick={onStart}>Begin independent practice</PrimaryButton></div></Card></section></Shell>;
}
...
function MixedMechanicalPracticeIntroScreen({ onStart }: { onStart: () => void }) {
  return (
    <Shell>
      <section className="mx-auto flex min-h-[82vh] max-w-3xl items-center px-6 py-12 sm:px-8 sm:py-16">
        <Card className="text-center">
          <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Integrated mechanical practice</p>
          <h1 className="mt-5 text-4xl font-semibold">Mixed Mechanical Practice</h1>
          <p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-[#9AA3B2]">
            Hydraulics, gears, pulleys and levers are now interleaved. The topic is not announced before each question, so your first task is to recognise which mechanical method applies.
          </p>
          <div className="mx-auto mt-9 grid max-w-xl gap-3 sm:grid-cols-3">
            {["12 questions", "Immediate feedback", "No topic labels before answering"].map((item) => (
              <div key={item} className="rounded-xl border border-white/5 bg-[#111418] p-4 text-sm text-[#AAB4C0]">{item}</div>
            ))}
          </div>
          <p className="mx-auto mt-7 max-w-xl text-sm leading-relaxed text-[#8D98A6]">
            After each answer, Vivalsa will name the method that applied. The aim is to practise choosing the right tool, not merely remembering four separate topics.
          </p>
          <div className="mt-10"><PrimaryButton onClick={onStart}>Start mixed practice</PrimaryButton></div>
        </Card>
      </section>
    </Shell>
  );
...
function MixedMechanicalQuestionScreen({ journey, sessionId, questionIndex, onAnswer }: { journey: MvpGuestJourney; sessionId: string; questionIndex: number; onAnswer: (response: AssessmentResponse, final: boolean) => void }) {
  ...
  const methodCues: Record<string, string> = {
    hydraulics: "Trace pressure through the fluid, then compare piston area, force or movement.",
    gears: "Find the driver, count the meshes, then compare tooth counts if speed matters.",
    pulleys: "Find the moving load, trace the rope and count the supporting strands.",
    levers: "Find the fulcrum, locate load and effort, then compare their distances.",
  };
  ...
  function next() {
    ...
    onAnswer(response, questionIndex === mixedMechanicalPracticeQuestions.length - 1);
  }
  ...
  return (
    <Shell right="Mixed practice">
      <section className="mx-auto max-w-5xl px-6 pb-20 pt-8 sm:px-8 sm:pb-12 sm:pt-10">
        ...
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Mixed Mechanical Practice</p>
            <h1 className="mt-3 text-3xl font-semibold">Choose the right method</h1>
          </div>
          <div className="text-right text-sm text-[#8D98A6]">
            Question {questionIndex + 1} of {mixedMechanicalPracticeQuestions.length}
            <br />
            <span className="text-xs">{answered} saved</span>
          </div>
        </div>
        ...
        <Card>
          <MixedMechanicalAssessmentDiagram question={question} />
          <p className="mt-6 text-xl leading-relaxed text-[#F4F6F8]">{question.stem}</p>
          ...
          {showFeedback && (
            <div className={`mt-5 rounded-2xl border p-5 ${
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <div className="mb-3 inline-flex rounded-full border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-[#5ED3F3]">
                    Method: {topicLabels[question.subcompetency] ?? "Mechanical reasoning"}
                  </div>
                  <p className="font-semibold">{selectedCorrect ? "Correct" : "Not quite"}</p>
                  <p className="mt-2 leading-relaxed text-[#C8D2DD]">{question.explanation}</p>
                  <p className="mt-3 text-sm leading-relaxed text-[#D9F8FF]">
                    <span className="text-[#6E7A88]">Reasoning cue: </span>
                    {methodCues[question.subcompetency] ?? question.feedbackCue}
                  </p>
                </div>
                <div className="shrink-0">
                  <PrimaryButton className="w-full sm:w-auto" onClick={next}>
                    {questionIndex === mixedMechanicalPracticeQuestions.length - 1 ? "Complete mixed practice" : "Next question"}
                  </PrimaryButton>
                </div>
              </div>
            </div>
...
function MixedMechanicalPracticeDebriefScreen({ journey, onWhy, onDashboard, onStartAssessment }: { journey: MvpGuestJourney; onWhy: () => void; onDashboard: () => void; onStartAssessment: () => void }) {
  const debrief = getLatestDebrief(journey);
  const rec = getCurrentRecommendation(journey);
  const summary = [...journey.practiceSummaries].reverse().find((item) => item.sessionType === "mixed_mechanical_practice");
  const labels: Record<string, string> = { hydraulics: "Hydraulics", gears: "Gears", pulleys: "Pulleys", levers: "Levers" };
  const canAssess = rec?.recommendationType === "begin_mixed_mechanical_assessment";

  return (
    <Shell>
      <section className="mx-auto flex min-h-[82vh] max-w-5xl items-center px-6 py-12 sm:px-8 sm:py-16">
        <Card>
          <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Mixed practice debrief</p>
          <h1 className="mt-5 text-4xl font-semibold leading-tight">{debrief?.title ?? "Mixed mechanical practice complete"}</h1>
          <p className="mt-5 text-lg leading-relaxed text-[#9AA3B2]">{debrief?.summary}</p>

          {summary && (
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {summary.conceptBreakdown.map((item) => (
                <div key={item.concept} className="rounded-2xl border border-white/5 bg-[#111418] p-5">
                  <div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">{labels[item.concept] ?? item.concept}</div>
                  <div className="mt-3 text-3xl font-semibold">{item.correct}/{item.attempted}</div>
                  <div className="mt-2 text-sm text-[#9AA3B2]">{Math.round(item.accuracy * 100)}%</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-2">
            <div className="rounded-2xl border border-white/5 bg-[#111418] p-6">
              <div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Interpretation</div>
              <p className="mt-3 leading-relaxed text-[#C8D2DD]">{debrief?.interpretation}</p>
            </div>
            <div className="rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/5 p-6">
              <div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Recommended next step</div>
              <h2 className="mt-3 text-2xl font-semibold">{rec?.title}</h2>
              <p className="mt-3 leading-relaxed text-[#AAB4C0]">{rec?.summary}</p>
            </div>
          </div>

          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <SecondaryButton onClick={onWhy}>Why this next step?</SecondaryButton>
            {canAssess && <PrimaryButton onClick={onStartAssessment}>Start Mixed Mechanical Assessment</PrimaryButton>}
...
function GearReasoningReminder() {
  return (
    <div className="mb-5 rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 px-4 py-3 sm:px-5">
      ...
      <p className="mt-1 text-sm leading-relaxed text-[#C8D2DD]">
        Find the driver <span className="text-[#6E7A88]">→</span> count the meshes <span className="text-[#6E7A88]">→</span> decide odd or even.
      </p>
    </div>
  );
}
...
function GuidedGearQuestionScreen({ journey, sessionId, questionIndex, onAnswer }: { journey: MvpGuestJourney; sessionId: string; questionIndex: number; onAnswer: (response: AssessmentResponse, final: boolean) => void }) {
  ...
  function next() {
    if (!selectedOptionId) return;
    const response = createAssessmentResponse(sessionId, question, selectedOptionId, Date.now() - startedAt, false);
    onAnswer(response, questionIndex === guidedGearPracticeQuestions.length - 1);
  }
  ...
  return (
    <Shell right="Guided gear practice">
      <section className="mx-auto max-w-5xl px-6 pb-20 pt-8 sm:px-8 sm:pb-12 sm:pt-10">
        ...
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Guided Gear Practice</p>
            <h1 className="mt-3 text-3xl font-semibold">Gear reasoning</h1>
          </div>
          <div className="text-right text-sm text-[#8D98A6]">
            Question {questionIndex + 1} of {guidedGearPracticeQuestions.length}
            <br />
            <span className="text-xs">{answered} saved</span>
          </div>
        </div>

        <GearReasoningReminder />
          ...
          {showFeedback && (
            <div className={`mt-5 rounded-2xl border p-5 ${
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="shrink-0">
                  <PrimaryButton className="w-full sm:w-auto" onClick={next}>
                    {questionIndex === guidedGearPracticeQuestions.length - 1 ? "Complete gear practice" : "Next question"}
                  </PrimaryButton>
                </div>
              </div>
            </div>
...
function GearIndependentPracticeQuestionScreen({ journey, sessionId, questionIndex, onAnswer }: { journey: MvpGuestJourney; sessionId: string; questionIndex: number; onAnswer: (response: AssessmentResponse, final: boolean) => void }) {
  ...
  const [selectedOptionId, setSelectedOptionId] = useState<string | null>(null);
  const [showFeedback, setShowFeedback] = useState(false);
  const pendingResponseRef = useRef<AssessmentResponse | null>(null);
  ...
  function select(optionId: string) {
    ...
    setSelectedOptionId(optionId);
    setShowFeedback(true);
    // Silent telemetry only. Independent Practice remains untimed for the learner.
    pendingResponseRef.current = createAssessmentResponse(sessionId, question, optionId, latencyMs, false, {
      responseTimeBasis: "selection_latency",
      firstActionTimeMs: latencyMs,
      telemetryVersion: gearDirectionFluencyProfile.telemetryVersion,
  ...
  function next() {
    ...
    if (!response) return;
    onAnswer(response, questionIndex === gearIndependentPracticeQuestions.length - 1);
  }
  ...
  return (
    <Shell right="Independent gear practice">
      <section className="mx-auto max-w-5xl px-6 pb-20 pt-8 sm:px-8 sm:pb-12 sm:pt-10">
        ...
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Independent Gear Practice</p>
            <h1 className="mt-3 text-3xl font-semibold">Apply the gear rules</h1>
          </div>
          <div className="text-right text-sm text-[#8D98A6]">
            Question {questionIndex + 1} of {gearIndependentPracticeQuestions.length}
            <br />
            <span className="text-xs">{answered} saved</span>
          </div>
        </div>
          ...
          {showFeedback && (
            <div className={`mt-5 rounded-2xl border p-5 ${
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="shrink-0">
                  <PrimaryButton className="w-full sm:w-auto" onClick={next}>
                    {questionIndex === gearIndependentPracticeQuestions.length - 1 ? "Complete independent practice" : "Next question"}
                  </PrimaryButton>
                </div>
              </div>
            </div>
...
function GearIndependentPracticeDebriefScreen({ journey, onWhy, onDashboard, onStartFluency, onReviewIncorrect, onReviewAll }: { journey: MvpGuestJourney; onWhy: () => void; onDashboard: () => void; onStartFluency: () => void; onReviewIncorrect: () => void; onReviewAll: () => void }) {
  const debrief = getLatestDebrief(journey);
  const rec = getCurrentRecommendation(journey);
  ...
  const directionEvidence = summary?.conceptBreakdown.find((item) => item.concept === "direction");
  const canTryFluency = Boolean(summary && summary.accuracy >= 0.8 && directionEvidence && directionEvidence.accuracy >= 0.8);
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-4xl items-center px-8 py-16"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Independent practice debrief</p><h1 className="mt-6 text-4xl font-semibold leading-tight">{debrief?.title ?? "Independent Gear Practice complete"}</h1><p className="mt-6 text-lg leading-relaxed text-[#9AA3B2]">{debrief?.summary}</p>{summary && <div className="mt-8 grid gap-4 sm:grid-cols-3">{summary.conceptBreakdown.map((item) => <div key={item.concept} className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">{labels[item.concept] ?? item.concept}</div><div className="mt-3 text-3xl font-semibold">{item.correct}/{item.attempted}</div><div className="mt-2 text-sm text-[#9AA3B2]">{Math.round(item.accuracy * 100)}%</div></div>)}</div>}<div className="mt-8 grid gap-5"><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Practice result</div><p className="mt-3 text-[#C8D2DD]">{debrief?.comparison}</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Interpretation</div><p className="mt-3 text-[#C8D2DD]">{debrief?.interpretation}</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Recommended next step</div><h2 className="mt-3 text-2xl font-semibold">{rec?.title}</h2><p className="mt-3 text-[#AAB4C0]">{rec?.summary}</p><div className="mt-5"><Badge>Moderate confidence</Badge></div></div></div><AnswerReviewActions incorrectCount={incorrectCount} onReviewIncorrect={onReviewIncorrect} onReviewAll={onReviewAll} /><div className="mt-9 flex flex-col gap-3 sm:flex-row"><SecondaryButton onClick={onWhy}>Why this next step?</SecondaryButton>{canTryFluency && <PrimaryButton onClick={onStartFluency}>Try Gear Direction Fluency</PrimaryButton>}<PrimaryButton onClick={onDashboard}>View dashboard</PrimaryButton></div></Card></section></Shell>;
}


function GearFluencyIntroScreen({ onStart, onSkip }: { onStart: () => void; onSkip: () => void }) {
  return <Shell><section className="mx-auto flex min-h-[82vh] max-w-3xl items-center px-6 py-12 sm:px-8 sm:py-16"><Card className="text-center"><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Fluency prototype</p><h1 className="mt-5 text-4xl font-semibold">Gear Direction Fluency</h1><p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-[#9AA3B2]">You already know the method. This short set starts measuring how efficiently you recognise direction patterns without asking you to rush.</p><div className="mx-auto mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">{["8 direction questions", "Accuracy still comes first", "35 s is a gentle guide, not a deadline"].map((item) => <div key={item} className="rounded-xl border border-white/5 bg-[#111418] p-4 text-sm text-[#AAB4C0]">{item}</div>)}</div><div className="mx-auto mt-7 max-w-xl rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/5 p-5 text-left"><div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#5ED3F3]">Fluency cue</div><p className="mt-2 text-sm leading-relaxed text-[#C8D2DD]">Count the <b>contacts</b>, not the gears. Even number of reversals = same direction. Odd number = opposite direction.</p></div><div className="mt-10 flex flex-col justify-center gap-3 sm:flex-row"><SecondaryButton onClick={onSkip}>Skip for now</SecondaryButton><PrimaryButton onClick={onStart}>Start fluency set</PrimaryButton></div></Card></section></Shell>;
}

function GearFluencyQuestionScreen({ journey, sessionId, questionIndex, onAnswer }: { journey: MvpGuestJourney; sessionId: string; questionIndex: number; onAnswer: (response: AssessmentResponse, final: boolean) => void }) {
  const [startedAt, setStartedAt] = useState(Date.now());
  const [selectedOptionId, setSelectedOptionId] = useState<string | null>(null);
  const [showFeedback, setShowFeedback] = useState(false);
  const [elapsedMs, setElapsedMs] = useState(0);
  const [selectionTimeMs, setSelectionTimeMs] = useState<number | null>(null);
...
function getGearFluencyBlockAnalytics(journey: MvpGuestJourney): GearFluencyBlockAnalytics[] {
  const completed = journey.sessions.filter((session) => session.sessionType === "gear_fluency" && session.completedAt);
  ...
  return completed.map((session) => {
    ...
    return {
      ...
      withinTarget: timingValid ? responses.filter((response) => response.correct && response.responseTimeMs <= GEAR_FLUENCY_INITIAL_TARGET_MS).length : 0,
      rapidErrors: timingValid ? responses.filter((response) => !response.correct && response.responseTimeMs <= rapidBoundaryMs).length : 0,
      deviceClasses: Array.from(new Set(responses.map((response) => response.deviceClass).filter(Boolean) as string[])),
    };
  });
}
...
function GearFluencyDebriefScreen({ journey, onGearCheck, onDashboard }: { journey: MvpGuestJourney; onGearCheck: () => void; onDashboard: () => void }) {
  ...
  const med = current?.medianMs ?? 0;
  const iqr = current?.iqrMs ?? 0;
  const withinTarget = current?.withinTarget ?? 0;
  const previousAccuracy = previous?.accuracy ?? null;
...
function MechanicalCalibrationPilotDiagram({ item }: { item: MechanicalCalibrationPilotItem }) {
  ...
  if (diagram.kind === "lever_balance") {
    ...
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label={`Lever with a ${diagram.loadN} newton load ${diagram.loadArmM} metres from the fulcrum and effort applied ${diagram.effortArmM} metres from the fulcrum.`}>
      <svg viewBox="0 0 740 300" className="h-auto w-full">
        <defs>
          <marker id="leverLoadArrow" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z" fill="#FFB86B"/></marker>
          <marker id="leverEffortArrow" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z" fill="#5ED3F3"/></marker>
        </defs>
        ...
        <line x1={loadX} y1="80" x2={loadX} y2="144" stroke="#FFB86B" strokeWidth="6" markerEnd="url(#leverLoadArrow)"/>
        <text x={loadX} y="65" textAnchor="middle" fill="#FFD6A8" fontSize="19" fontWeight="600">Load {diagram.loadN} N</text>
        <line x1={effortX} y1="80" x2={effortX} y2="144" stroke="#5ED3F3" strokeWidth="6" markerEnd="url(#leverEffortArrow)"/>
  ...
  if (diagram.kind === "linked_bars") {
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label="Two pivoted upright bars connected by a rigid horizontal link. The top of the left bar is pushed to the right.">
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <line x1="120" y1="270" x2="620" y2="270" stroke="#6E7A88" strokeWidth="8" strokeLinecap="round"/>
        <line x1="245" y1="250" x2="245" y2="92" stroke="#DDE3EA" strokeWidth="12" strokeLinecap="round"/>
        <line x1="495" y1="250" x2="495" y2="92" stroke="#DDE3EA" strokeWidth="12" strokeLinecap="round"/>
        <circle cx="245" cy="250" r="12" fill="#111418" stroke="#8D98A6" strokeWidth="4"/><circle cx="495" cy="250" r="12" fill="#111418" stroke="#8D98A6" strokeWidth="4"/>
        <line x1="245" y1="170" x2="495" y2="170" stroke="#5ED3F3" strokeWidth="8" strokeLinecap="round"/>
        <circle cx="245" cy="170" r="8" fill="#D9F8FF"/><circle cx="495" cy="170" r="8" fill="#D9F8FF"/>
        <line x1="135" y1="92" x2="220" y2="92" stroke="#FFB86B" strokeWidth="6" strokeLinecap="round"/>
        <polygon points="220,82 220,102 238,92" fill="#FFB86B"/>
        <text x="135" y="68" fill="#FFD6A8" fontSize="18" fontWeight="600">Push right</text>
        <text x="495" y="68" textAnchor="middle" fill="#D9F8FF" fontSize="18" fontWeight="600">Top of right bar ?</text>
        <text x="370" y="205" textAnchor="middle" fill="#5ED3F3" fontSize="16">Rigid link</text>
        <text x="245" y="298" textAnchor="middle" fill="#8D98A6" fontSize="15">pivot</text><text x="495" y="298" textAnchor="middle" fill="#8D98A6" fontSize="15">pivot</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "bell_crank") {
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label="Right-angle bell crank pivoted at its corner. The left end is pushed downward; the top-end movement is unknown.">
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <line x1="230" y1="195" x2="405" y2="195" stroke="#DDE3EA" strokeWidth="14" strokeLinecap="round"/>
        <line x1="405" y1="195" x2="405" y2="70" stroke="#DDE3EA" strokeWidth="14" strokeLinecap="round"/>
        <circle cx="405" cy="195" r="15" fill="#111418" stroke="#8D98A6" strokeWidth="5"/>
        <line x1="230" y1="90" x2="230" y2="165" stroke="#FFB86B" strokeWidth="6" strokeLinecap="round"/>
        <polygon points="220,158 240,158 230,176" fill="#FFB86B"/>
        <text x="230" y="66" textAnchor="middle" fill="#FFD6A8" fontSize="18" fontWeight="600">Push down</text>
        <text x="405" y="42" textAnchor="middle" fill="#D9F8FF" fontSize="18" fontWeight="600">Top end ?</text>
        <text x="405" y="236" textAnchor="middle" fill="#8D98A6" fontSize="15">fixed pivot</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "inclined_planes") {
    const pxPerM = 75;
    const rise = diagram.heightM * pxPerM;
    const shortHyp = diagram.shortLengthM * pxPerM;
    const longHyp = diagram.longLengthM * pxPerM;
    const shortBase = Math.sqrt(Math.max(0, shortHyp * shortHyp - rise * rise));
    const longBase = Math.sqrt(Math.max(0, longHyp * longHyp - rise * rise));
    const baseY = 238;
    const topY = baseY - rise;
    const shortLeft = 55;
    const longLeft = 350;
    const shortRight = shortLeft + shortBase;
    const longRight = longLeft + longBase;
    const shortAngle = -Math.atan2(rise, shortBase) * 180 / Math.PI;
    const longAngle = -Math.atan2(rise, longBase) * 180 / Math.PI;
    const loadT = 0.56;
    const shortLoadX = shortLeft + shortBase * loadT;
    const shortLoadY = baseY - rise * loadT;
    const longLoadX = longLeft + longBase * loadT;
    const longLoadY = baseY - rise * loadT;
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label={`Two ramps drawn to the same scale. Both rise ${diagram.heightM} metre. Ramp A is ${diagram.shortLengthM} metres long and Ramp B is ${diagram.longLengthM} metres long.`}>
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <text x={(shortLeft + shortRight) / 2} y="42" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">Ramp A · {diagram.shortLengthM} m</text>
        <path d={`M${shortLeft} ${baseY} L${shortRight} ${baseY} L${shortRight} ${topY} Z`} fill="#17212A" stroke="#8D98A6" strokeWidth="4"/>
        <g transform={`translate(${shortLoadX} ${shortLoadY}) rotate(${shortAngle})`}><rect x="-30" y="-40" width="60" height="40" rx="6" fill="#20313D" stroke="#FFB86B" strokeWidth="4"/><text x="0" y="-14" textAnchor="middle" fill="#FFD6A8" fontSize="15" fontWeight="600">Load</text></g>
        <line x1={shortRight} y1={topY} x2={shortRight} y2={baseY} stroke="#5ED3F3" strokeWidth="3" strokeDasharray="7 7"/>

        <text x={(longLeft + longRight) / 2} y="42" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">Ramp B · {diagram.longLengthM} m</text>
        <path d={`M${longLeft} ${baseY} L${longRight} ${baseY} L${longRight} ${topY} Z`} fill="#17212A" stroke="#8D98A6" strokeWidth="4"/>
        <g transform={`translate(${longLoadX} ${longLoadY}) rotate(${longAngle})`}><rect x="-30" y="-40" width="60" height="40" rx="6" fill="#20313D" stroke="#FFB86B" strokeWidth="4"/><text x="0" y="-14" textAnchor="middle" fill="#FFD6A8" fontSize="15" fontWeight="600">Load</text></g>
        <line x1={longRight} y1={topY} x2={longRight} y2={baseY} stroke="#5ED3F3" strokeWidth="3" strokeDasharray="7 7"/>

        <text x="370" y="185" textAnchor="middle" fill="#8D98A6" fontSize="16">same rise · {diagram.heightM} m</text>
        <text x="370" y="292" textAnchor="middle" fill="#8D98A6" fontSize="16">Same load · same {diagram.heightM} m rise · friction ignored · ramps drawn to one scale</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "wheel_axle_compare") {
    const sets = [
      { x: 230, label: "A", wr: 78, ar: 22, wheel: diagram.aWheelRadiusCm, axle: diagram.aAxleRadiusCm },
      { x: 510, label: "B", wr: 62, ar: 31, wheel: diagram.bWheelRadiusCm, axle: diagram.bAxleRadiusCm },
    ];
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label="Two wheel-and-axle arrangements with different wheel and axle radii.">
      <svg viewBox="0 0 740 330" className="h-auto w-full">
        {sets.map((s) => <g key={s.label}><text x={s.x} y="40" textAnchor="middle" fill="#D9F8FF" fontSize="22" fontWeight="600">Arrangement {s.label}</text><circle cx={s.x} cy="150" r={s.wr} fill="#17212A" stroke="#5ED3F3" strokeWidth="5"/><circle cx={s.x} cy="150" r={s.ar} fill="#20313D" stroke="#D9F8FF" strokeWidth="4"/><circle cx={s.x} cy="150" r="7" fill="#D9F8FF"/><text x={s.x} y="255" textAnchor="middle" fill="#C8D2DD" fontSize="17">wheel radius {s.wheel} cm</text><text x={s.x} y="280" textAnchor="middle" fill="#AAB4C0" fontSize="17">axle radius {s.axle} cm</text></g>)}
        <text x="370" y="315" textAnchor="middle" fill="#8D98A6" fontSize="15">Effort is applied at the outer wheel rim</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "friction_compare") {
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label={`Two identical ${diagram.loadN} newton blocks, one on a smooth surface and one on a rough surface.`}>
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <text x="200" y="42" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">A · smooth surface</text><rect x="145" y="120" width="110" height="80" rx="8" fill="#20313D" stroke="#D9F8FF" strokeWidth="4"/><text x="200" y="166" textAnchor="middle" fill="#D9F8FF" fontSize="18">{diagram.loadN} N</text><line x1="80" y1="220" x2="320" y2="220" stroke="#8D98A6" strokeWidth="7" strokeLinecap="round"/>
        <text x="540" y="42" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">B · rough surface</text><rect x="485" y="120" width="110" height="80" rx="8" fill="#20313D" stroke="#D9F8FF" strokeWidth="4"/><text x="540" y="166" textAnchor="middle" fill="#D9F8FF" fontSize="18">{diagram.loadN} N</text><path d="M410 220 l15 -9 l15 9 l15 -9 l15 9 l15 -9 l15 9 l15 -9 l15 9 l15 -9 l15 9 l15 -9 l15 9 l15 -9 l15 9 l15 -9 l15 9" fill="none" stroke="#8D98A6" strokeWidth="5"/>
        <line x1="200" y1="90" x2="270" y2="90" stroke="#FFB86B" strokeWidth="5"/><polygon points="270,81 270,99 286,90" fill="#FFB86B"/><line x1="540" y1="90" x2="610" y2="90" stroke="#FFB86B" strokeWidth="5"/><polygon points="610,81 610,99 626,90" fill="#FFB86B"/><text x="370" y="286" textAnchor="middle" fill="#8D98A6" fontSize="16">Which needs the larger force to START moving?</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "stability_compare") {
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label="Three objects with different base widths and centre-of-mass heights, labelled A, B and C.">
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <line x1="65" y1="260" x2="675" y2="260" stroke="#6E7A88" strokeWidth="7" strokeLinecap="round"/>
        <rect x="120" y="90" width="75" height="170" rx="5" fill="#20313D" stroke="#9AA3B2" strokeWidth="4"/><circle cx="158" cy="175" r="7" fill="#FFB86B"/><text x="158" y="55" textAnchor="middle" fill="#D9F8FF" fontSize="22" fontWeight="600">A</text>
        <rect x="285" y="180" width="170" height="80" rx="5" fill="#20313D" stroke="#5ED3F3" strokeWidth="4"/><circle cx="370" cy="220" r="7" fill="#FFB86B"/><text x="370" y="145" textAnchor="middle" fill="#D9F8FF" fontSize="22" fontWeight="600">B</text>
        <rect x="535" y="125" width="115" height="135" rx="5" fill="#20313D" stroke="#9AA3B2" strokeWidth="4"/><circle cx="593" cy="192" r="7" fill="#FFB86B"/><text x="593" y="90" textAnchor="middle" fill="#D9F8FF" fontSize="22" fontWeight="600">C</text>
        <text x="370" y="303" textAnchor="middle" fill="#8D98A6" fontSize="15">Orange dot = approximate centre of mass</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "trolley_inertia") {
    const right = diagram.accelerationDirection === "right";
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label={`A block rests freely on a trolley. It is not attached and can slide. The trolley suddenly accelerates ${diagram.accelerationDirection}.`}>
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <rect x="180" y="165" width="380" height="70" rx="12" fill="#20313D" stroke="#5ED3F3" strokeWidth="4"/><circle cx="255" cy="255" r="24" fill="#111418" stroke="#9AA3B2" strokeWidth="6"/><circle cx="485" cy="255" r="24" fill="#111418" stroke="#9AA3B2" strokeWidth="6"/>
        <rect x="310" y="95" width="120" height="70" rx="8" fill="#17212A" stroke="#FFB86B" strokeWidth="4"/><text x="370" y="124" textAnchor="middle" fill="#FFD6A8" fontSize="17" fontWeight="600">Block</text><text x="370" y="148" textAnchor="middle" fill="#FFD6A8" fontSize="13">not attached</text>
        <line x1="325" y1="188" x2="415" y2="188" stroke="#D9F8FF" strokeWidth="3" strokeLinecap="round"/><polygon points="325,188 337,181 337,195" fill="#D9F8FF"/><polygon points="415,188 403,181 403,195" fill="#D9F8FF"/><text x="370" y="214" textAnchor="middle" fill="#D9F8FF" fontSize="14">free to slide</text>
        <line x1={right ? 235 : 505} y1="55" x2={right ? 485 : 255} y2="55" stroke="#5ED3F3" strokeWidth="7" strokeLinecap="round"/><polygon points={right ? "485,44 485,66 505,55" : "255,44 255,66 235,55"} fill="#5ED3F3"/>
        <text x="370" y="35" textAnchor="middle" fill="#D9F8FF" fontSize="18" fontWeight="600">Trolley suddenly accelerates {diagram.accelerationDirection} {right ? "→" : "←"}</text>
        <text x="370" y="304" textAnchor="middle" fill="#8D98A6" fontSize="15">Friction negligible · answer is motion RELATIVE TO the trolley</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "shaft_view") {
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label={`A shaft viewed from opposite ends A and B. From end A it appears to rotate ${diagram.endADirection}.`}>
      <svg viewBox="0 0 740 320" className="h-auto w-full">
        <ellipse cx="210" cy="160" rx="55" ry="82" fill="#17212A" stroke="#5ED3F3" strokeWidth="5"/><rect x="210" y="78" width="320" height="164" fill="#17212A" stroke="#8D98A6" strokeWidth="4"/><ellipse cx="530" cy="160" rx="55" ry="82" fill="#20313D" stroke="#9AA3B2" strokeWidth="5"/>
        <line x1="210" y1="160" x2="530" y2="160" stroke="#D9F8FF" strokeWidth="4" strokeDasharray="10 8"/>
        <text x="115" y="55" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">View from A</text><text x="625" y="55" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">View from B</text>
        <text x="115" y="170" textAnchor="middle" fill="#5ED3F3" fontSize="58">{directionGlyph(diagram.endADirection)}</text><text x="625" y="170" textAnchor="middle" fill="#5ED3F3" fontSize="46">?</text>
        <text x="370" y="292" textAnchor="middle" fill="#8D98A6" fontSize="16">Same rotating shaft · opposite viewing directions</text>
      </svg>
    </div>;
  }

  if (diagram.kind === "integrated_gear_belt") {
    const rA = diagram.aTeeth * 1.35;
    const rB = diagram.bTeeth * 1.35;
    const gearAx = 170;
    const gearBx = gearAx + rA + rB;
    const cy = 145;
    const pulleyCx = gearBx;
    const pulleyDx = 590;
    const pulleyR = 42;
    return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label="Integrated machine: Gear A drives Gear B. Gear B shares a shaft with pulley C, which drives pulley D through an open belt.">
      <svg viewBox="0 0 740 330" className="h-auto w-full">
        <MechanicalGearGlyph cx={gearAx} cy={cy} teeth={diagram.aTeeth} pitchRadius={rA} stroke="#5ED3F3"/><MechanicalGearGlyph cx={gearBx} cy={cy} teeth={diagram.bTeeth} pitchRadius={rB} stroke="#9AA3B2"/>
        <text x={gearAx} y="55" textAnchor="middle" fill="#D9F8FF" fontSize="18" fontWeight="600">Gear A</text><text x={gearAx} y="80" textAnchor="middle" fill="#5ED3F3" fontSize="24">{directionGlyph(diagram.driverDirection)}</text><text x={gearBx} y="55" textAnchor="middle" fill="#D9F8FF" fontSize="18" fontWeight="600">Gear B</text>
        <line x1={gearBx} y1={cy} x2={gearBx} y2="270" stroke="#6E7A88" strokeWidth="8" strokeLinecap="round"/><circle cx={pulleyCx} cy="250" r={pulleyR} fill="#17212A" stroke="#D9F8FF" strokeWidth="4"/><circle cx={pulleyDx} cy="250" r={pulleyR} fill="#17212A" stroke="#9AA3B2" strokeWidth="4"/>
        <line x1={pulleyCx} y1={208} x2={pulleyDx} y2={208} stroke="#DDE3EA" strokeWidth="8" strokeLinecap="round"/><line x1={pulleyCx} y1={292} x2={pulleyDx} y2={292} stroke="#DDE3EA" strokeWidth="8" strokeLinecap="round"/>
        <text x={pulleyCx} y="315" textAnchor="middle" fill="#D9F8FF" fontSize="17">Pulley C · same shaft as B</text><text x={pulleyDx} y="315" textAnchor="middle" fill="#D9F8FF" fontSize="17">Pulley D · ?</text><text x="500" y="192" textAnchor="middle" fill="#8D98A6" fontSize="15">open belt</text>
      </svg>
    </div>;
  }
  ...
  const rightX = 535;
  const cy = 145;
  return <div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-4" role="img" aria-label={`${diagram.crossed ? "Crossed" : "Open"} belt drive. Pulley A is the driver and turns ${diagram.driverDirection}.`}><svg viewBox="0 0 740 300" className="h-auto w-full">{diagram.crossed ? <><line x1={leftX} y1={cy-rA} x2={rightX} y2={cy+rB} stroke="#DDE3EA" strokeWidth="9" strokeLinecap="round"/><line x1={leftX} y1={cy+rA} x2={rightX} y2={cy-rB} stroke="#DDE3EA" strokeWidth="9" strokeLinecap="round"/></> : <><line x1={leftX} y1={cy-rA} x2={rightX} y2={cy-rB} stroke="#DDE3EA" strokeWidth="9" strokeLinecap="round"/><line x1={leftX} y1={cy+rA} x2={rightX} y2={cy+rB} stroke="#DDE3EA" strokeWidth="9" strokeLinecap="round"/></>}<circle cx={leftX} cy={cy} r={rA} fill="#171C23" stroke="#5ED3F3" strokeWidth="5"/><circle cx={rightX} cy={cy} r={rB} fill="#171C23" stroke="#9AA3B2" strokeWidth="5"/><circle cx={leftX} cy={cy} r="7" fill="#D9F8FF"/><circle cx={rightX} cy={cy} r="7" fill="#D9F8FF"/><text x={leftX} y="62" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">Pulley A · driver</text><text x={leftX} y="88" textAnchor="middle" fill="#5ED3F3" fontSize="24">{directionGlyph(diagram.driverDirection)}</text><text x={rightX} y="62" textAnchor="middle" fill="#D9F8FF" fontSize="20" fontWeight="600">Pulley B</text><text x="370" y="272" textAnchor="middle" fill="#8D98A6" fontSize="17">{diagram.crossed ? "Crossed belt" : "Open belt"}</text></svg></div>;
}

function InternalMechanicalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  type PilotSet = "rotation" | "force_systems" | "extensions";
  ...
  const activeItems = pilotSet === "rotation" ? mechanicalCalibrationRotationItems : pilotSet === "force_systems" ? mechanicalCalibrationForceSystemsItems : mechanicalCalibrationExtensionItems;
  ...
  function startRun(nextSet: PilotSet = "extensions") {
    setPilotSet(nextSet);
    setQuestionIndex(0);
    setSelectedOptionId(null);
    setResponses([]);
  ...
  function next() {
    ...
    if (questionIndex === activeItems.length - 1) {
      ...
      saveMechanicalCalibrationPilotRun(run);
      setPhase("debrief");
      return;
    }
  ...
  if (phase === "intro") {
    ...
    return <Shell right="Internal mechanical calibration"><section className="mx-auto max-w-5xl px-6 py-10 sm:px-8 sm:py-12"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only mechanical lab · v0.8</p><h1 className="mt-4 text-4xl font-semibold">Mechanical calibration authoring</h1><p className="mt-5 max-w-3xl leading-relaxed text-[#9AA3B2]">v0.8 preserves the rotating and force-system sets and adds the final nine Mechanical blueprint families: linkages, simple machines, friction/stability/inertia, viewpoint rotation and an integrated machine.</p><div className="mt-7 grid gap-4 lg:grid-cols-3"><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live re-check</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Rotating systems</div><div className="mt-2 text-sm leading-relaxed text-[#9AA3B2]">8 items · gear ratio, compound gears, open belts, crossed belts.</div><SecondaryButton className="mt-5" onClick={() => startRun("rotation")}>Re-run 8-item set</SecondaryButton></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Force systems</div><div className="mt-2 text-sm leading-relaxed text-[#AAB4C0]">12 items · pulleys, levers and hydraulics, including the latest lever-arrow correction.</div><SecondaryButton className="mt-5" onClick={() => startRun("force_systems")}>Re-run 12-item set</SecondaryButton></div><div className="rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Mechanical extensions</div><div className="mt-2 text-sm leading-relaxed text-[#AAB4C0]">9 items · linkages, ramps, wheel/axle, friction, stability, inertia, 3D viewpoint and integrated motion.</div><PrimaryButton className="mt-5" onClick={() => startRun("extensions")}>Re-run 9-item extensions set</PrimaryButton></div></div><div className="mt-6 rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/5 p-5 text-sm leading-relaxed text-[#C8D2DD]">All 20 Mechanical blueprint specifications have now passed device QA. Re-runs are for regression checking and item-level timing/distractor observations; author timing ranges remain provisional.</div><div className="mt-8 flex flex-col gap-3 sm:flex-row"><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton>{previousRuns.length > 0 && <SecondaryButton onClick={downloadMechanicalCalibrationPilotCsv}>Export {previousRuns.length} prior run{previousRuns.length === 1 ? "" : "s"}</SecondaryButton>}</div></Card></section></Shell>;
  }
...
function NumericalCalibrationPilotDiagram({ diagram }: { diagram?: NumericalCalibrationDiagram }) {
  ...
  if (diagram.kind === "composite_area") {
    const x = 140, y = 45, w = 360, h = 270;
    const cutW = w * (diagram.cutoutWidthM / diagram.outerWidthM);
    const cutH = h * (diagram.cutoutHeightM / diagram.outerHeightM);
    return <div className="mb-6 overflow-hidden rounded-2xl border border-white/5 bg-[#0E1115] p-4"><svg viewBox="0 0 700 380" className="w-full" role="img" aria-label="Rectangular floor with a corner service bay excluded"><rect x={x} y={y} width={w} height={h} rx="4" fill="#17303A" stroke={cyan} strokeWidth="4"/><rect x={x+w-cutW} y={y} width={cutW} height={cutH} fill={panel} stroke={orange} strokeWidth="3" strokeDasharray="8 6"/><text x={x+w-cutW/2} y={y+cutH/2-8} fill={orange} fontSize="20" textAnchor="middle" fontWeight="700">Excluded bay</text><text x={x+w-cutW/2} y={y+cutH/2+22} fill={text} fontSize="18" textAnchor="middle">{diagram.cutoutWidthM} m × {diagram.cutoutHeightM} m</text><line x1={x} y1={y-18} x2={x+w} y2={y-18} stroke={line} strokeWidth="2"/><line x1={x} y1={y-27} x2={x} y2={y-9} stroke={line} strokeWidth="2"/><line x1={x+w} y1={y-27} x2={x+w} y2={y-9} stroke={line} strokeWidth="2"/><text x={x+w/2} y={y-28} fill={text} fontSize="20" textAnchor="middle">{diagram.outerWidthM} m</text><line x1={x-22} y1={y} x2={x-22} y2={y+h} stroke={line} strokeWidth="2"/><line x1={x-31} y1={y} x2={x-13} y2={y} stroke={line} strokeWidth="2"/><line x1={x-31} y1={y+h} x2={x-13} y2={y+h} stroke={line} strokeWidth="2"/><text x={x-58} y={y+h/2} fill={text} fontSize="20" textAnchor="middle" transform={`rotate(-90 ${x-58} ${y+h/2})`}>{diagram.outerHeightM} m</text><text x="570" y="175" fill={muted} fontSize="18">Usable floor</text><text x="570" y="202" fill={muted} fontSize="18">= whole rectangle</text><text x="570" y="229" fill={muted} fontSize="18">− excluded bay</text></svg></div>;
  }

  if (diagram.kind === "rectangular_tank") {
    return <div className="mb-6 overflow-hidden rounded-2xl border border-white/5 bg-[#0E1115] p-4"><svg viewBox="0 0 700 360" className="w-full" role="img" aria-label="Rectangular tank with length width and height dimensions"><polygon points="225,95 470,95 535,55 290,55" fill="#1B3540" stroke={cyan} strokeWidth="3"/><polygon points="470,95 535,55 535,235 470,275" fill="#14262F" stroke={cyan} strokeWidth="3"/><rect x="225" y="95" width="245" height="180" fill="#17303A" stroke={cyan} strokeWidth="3"/><text x="350" y="188" fill={text} fontSize="22" textAnchor="middle" fontWeight="700">Rectangular tank</text><line x1="225" y1="305" x2="470" y2="305" stroke={orange} strokeWidth="3"/><line x1="225" y1="295" x2="225" y2="315" stroke={orange} strokeWidth="3"/><line x1="470" y1="295" x2="470" y2="315" stroke={orange} strokeWidth="3"/><text x="348" y="338" fill={orange} fontSize="20" textAnchor="middle">length {diagram.lengthM} m</text><line x1="195" y1="95" x2="195" y2="275" stroke={line} strokeWidth="3"/><line x1="185" y1="95" x2="205" y2="95" stroke={line} strokeWidth="3"/><line x1="185" y1="275" x2="205" y2="275" stroke={line} strokeWidth="3"/><text x="150" y="190" fill={text} fontSize="20" textAnchor="middle" transform="rotate(-90 150 190)">height {diagram.heightM} m</text><line x1="485" y1="83" x2="550" y2="43" stroke={line} strokeWidth="3"/><text x="570" y="55" fill={text} fontSize="20">width {diagram.widthM} m</text></svg></div>;
  }

  if (diagram.kind === "plan_scale") {
    return <div className="mb-6 overflow-hidden rounded-2xl border border-white/5 bg-[#0E1115] p-4"><svg viewBox="0 0 700 260" className="w-full" role="img" aria-label="Plan line measured at 7.2 centimetres with scale one to five hundred"><text x="350" y="44" fill={text} fontSize="22" textAnchor="middle" fontWeight="700">Plan measurement</text><line x1="135" y1="120" x2="565" y2="120" stroke={cyan} strokeWidth="8" strokeLinecap="round"/><line x1="135" y1="92" x2="135" y2="148" stroke={line} strokeWidth="3"/><line x1="565" y1="92" x2="565" y2="148" stroke={line} strokeWidth="3"/><text x="350" y="92" fill={orange} fontSize="22" textAnchor="middle">{diagram.planDistanceCm} cm on plan</text><rect x="245" y="170" width="210" height="58" rx="12" fill="#151A20" stroke="#5ED3F3" strokeOpacity="0.35"/><text x="350" y="206" fill={text} fontSize="22" textAnchor="middle">Scale 1 : {diagram.scaleDenominator}</text></svg></div>;
  }

  if (diagram.kind === "map_grid_route") {
    const left=145, top=35, cell=52, cols=6, rows=5;
    const startX=left, startY=top+rows*cell;
    const cornerX=startX+diagram.blocksEast*cell;
    const endY=startY-diagram.blocksNorth*cell;
    return <div className="mb-6 overflow-hidden rounded-2xl border border-white/5 bg-[#0E1115] p-4"><svg viewBox="0 0 700 380" className="w-full" role="img" aria-label="Grid map route travelling three blocks east then four blocks north">{Array.from({length: cols+1}).map((_,i)=><line key={`v${i}`} x1={left+i*cell} y1={top} x2={left+i*cell} y2={top+rows*cell} stroke="#33404B" strokeWidth="2"/>)}{Array.from({length: rows+1}).map((_,i)=><line key={`h${i}`} x1={left} y1={top+i*cell} x2={left+cols*cell} y2={top+i*cell} stroke="#33404B" strokeWidth="2"/>)}<polyline points={`${startX},${startY} ${cornerX},${startY} ${cornerX},${endY}`} fill="none" stroke={orange} strokeWidth="8" strokeLinejoin="round" strokeLinecap="round"/><circle cx={startX} cy={startY} r="10" fill={cyan}/><circle cx={cornerX} cy={endY} r="10" fill="#38D39F"/><text x={startX-18} y={startY+32} fill={text} fontSize="18">Start</text><text x={cornerX+18} y={endY+7} fill={text} fontSize="18">Finish</text><text x="500" y="120" fill={muted} fontSize="18">Each block</text><text x="500" y="148" fill={text} fontSize="22" fontWeight="700">{diagram.metresPerBlock} m</text><text x="500" y="205" fill={muted} fontSize="18">Follow the</text><text x="500" y="233" fill={orange} fontSize="20" fontWeight="700">marked route</text></svg></div>;
  }

  if (diagram.kind === "bar_chart") {
    const max=Math.max(...diagram.categories.map(c=>c.value), 60);
    const chartX=120, chartY=45, chartW=430, chartH=245;
    return <div className="mb-6 overflow-hidden rounded-2xl border border-white/5 bg-[#0E1115] p-4"><svg viewBox="0 0 700 360" className="w-full" role="img" aria-label="Bar chart comparing completed inspections for teams A B and C"><line x1={chartX} y1={chartY} x2={chartX} y2={chartY+chartH} stroke={line} strokeWidth="3"/><line x1={chartX} y1={chartY+chartH} x2={chartX+chartW} y2={chartY+chartH} stroke={line} strokeWidth="3"/>{[0,20,40,60].map(v=>{const yy=chartY+chartH-(v/max)*chartH; return <g key={v}><line x1={chartX-8} y1={yy} x2={chartX+chartW} y2={yy} stroke="#2C343D" strokeWidth="1"/><text x={chartX-18} y={yy+6} fill={muted} fontSize="16" textAnchor="end">{v}</text></g>})}{diagram.categories.map((c,i)=>{const bw=72, gap=48, bx=chartX+75+i*(bw+gap), bh=(c.value/max)*chartH, by=chartY+chartH-bh; return <g key={c.label}><rect x={bx} y={by} width={bw} height={bh} rx="7" fill={i===1?cyan:"#718094"}/><text x={bx+bw/2} y={by-10} fill={text} fontSize="18" textAnchor="middle">{c.value}</text><text x={bx+bw/2} y={chartY+chartH+30} fill={text} fontSize="18" textAnchor="middle">Team {c.label}</text></g>})}<text x="335" y="340" fill={muted} fontSize="17" textAnchor="middle">Completed inspections</text></svg></div>;
  }

  if (diagram.kind === "line_graph") {
    const minY=Math.min(...diagram.points.map(p=>p.value));
    const maxY=Math.max(...diagram.points.map(p=>p.value));
    const x0=125, y0=300, w=430, h=225;
    const maxMinute=Math.max(...diagram.points.map(p=>p.minute));
    const yPad=20;
    const yMin=minY-yPad, yMax=maxY+yPad;
    const px=(m:number)=>x0+(m/maxMinute)*w;
    const py=(v:number)=>y0-((v-yMin)/(yMax-yMin))*h;
    const points=diagram.points.map(p=>`${px(p.minute)},${py(p.value)}`).join(" ");
  ...
  function next() {
    ...
    if (questionIndex === activeItems.length - 1) {
      saveNumericalCalibrationPilotRun({
        ...
        pilotVersion: `${NUMERICAL_CALIBRATION_PILOT_VERSION}:${pilotSet}`,
        startedAt: runStartedAt ?? now(),
        completedAt: now(),
        responses,
      });
...
function AbstractLogicalCalibrationMarkSvg({ mark }: { mark: AbstractLogicalMark }) {
  const stroke = "#DCE3EA";
  const fill = "filled" in mark && mark.filled === false ? "none" : "#DCE3EA";
  const strokeWidth = 4;
  if (mark.kind === "circle") {
    return <circle cx={mark.x} cy={mark.y} r={(mark.size ?? 13) / 2} fill={fill} stroke={stroke} strokeWidth={strokeWidth} />;
  }
  if (mark.kind === "square") {
    const size = mark.size ?? 24;
    return <rect x={mark.x - size / 2} y={mark.y - size / 2} width={size} height={size} rx="2" fill={fill} stroke={stroke} strokeWidth={strokeWidth} transform={`rotate(${mark.rotation ?? 0} ${mark.x} ${mark.y})`} />;
  }
  if (mark.kind === "diamond") {
    const size = mark.size ?? 24;
    return <rect x={mark.x - size / 2} y={mark.y - size / 2} width={size} height={size} rx="2" fill={fill} stroke={stroke} strokeWidth={strokeWidth} transform={`rotate(45 ${mark.x} ${mark.y})`} />;
  }
  if (mark.kind === "triangle") {
    const size = mark.size ?? 28;
    const half = size / 2;
    const top = mark.y - half;
    const bottom = mark.y + half * 0.75;
    return <polygon points={`${mark.x},${top} ${mark.x - half},${bottom} ${mark.x + half},${bottom}`} fill={fill} stroke={stroke} strokeWidth={strokeWidth} strokeLinejoin="round" transform={`rotate(${mark.rotation ?? 0} ${mark.x} ${mark.y})`} />;
  }
  if (mark.kind === "arrow") {
    const size = mark.size ?? 30;
    return <g transform={`rotate(${mark.rotation ?? 0} ${mark.x} ${mark.y})`}><line x1={mark.x} y1={mark.y + size * 0.58} x2={mark.x} y2={mark.y - size * 0.18} stroke={stroke} strokeWidth="6" strokeLinecap="round"/><polygon points={`${mark.x},${mark.y - size * 0.72} ${mark.x - size * 0.34},${mark.y - size * 0.12} ${mark.x + size * 0.34},${mark.y - size * 0.12}`} fill={stroke}/></g>;
  }
  if (mark.kind === "line") {
...
function AbstractLogicalCalibrationCellView({ cell, compact = false }: { cell: AbstractLogicalCell | null; compact?: boolean }) {
  return <div className={`flex aspect-square items-center justify-center rounded-2xl border border-white/10 bg-[#111418] ${compact ? "p-1" : "p-2"}`}>
    {cell ? <svg viewBox="0 0 100 100" className="h-full w-full" aria-hidden="true">{cell.marks.map((mark, index) => <g key={index}><AbstractLogicalCalibrationMarkSvg mark={mark}/></g>)}</svg> : <span className="text-3xl font-semibold text-[#5ED3F3]">?</span>}
  </div>;
}
...
function AbstractLogicalCalibrationDiagramView({ diagram }: { diagram: AbstractLogicalCalibrationDiagram }) {
  ...
  if (diagram.kind === "rule_machine") {
    return <div className="mb-7 rounded-3xl border border-white/5 bg-[#0F1216] p-4 sm:p-6"><div className="mx-auto max-w-xl"><div className="mb-2 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-end gap-2 text-center text-xs font-semibold uppercase tracking-[0.12em] text-[#6E7A88]"><span>Input</span><span></span><span>Output</span></div><div className="space-y-3">{diagram.examples.map((example, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2"><AbstractLogicalCalibrationCellView cell={example.input} compact/><span className="text-2xl text-[#5ED3F3]">→</span><AbstractLogicalCalibrationCellView cell={example.output} compact/></div>)}<div className="border-t border-white/10 pt-3"><div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2"><AbstractLogicalCalibrationCellView cell={diagram.target} compact/><span className="text-2xl text-[#5ED3F3]">→</span><AbstractLogicalCalibrationCellView cell={null} compact/></div></div></div></div></div>;
  }
...
type AbstractLogicalCalibrationPilotSet = "sequence_matrix" | "classification_deduction" | "completion";

function InternalAbstractLogicalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  const [phase, setPhase] = useState<"intro" | "question" | "debrief">("intro");
  const [pilotSet, setPilotSet] = useState<AbstractLogicalCalibrationPilotSet>("completion");
  const [questionIndex, setQuestionIndex] = useState(0);
  ...
  const [runStartedAt, setRunStartedAt] = useState<string | null>(null);
  const questionStartedAtRef = useRef(0);
  const activeItems = pilotSet === "completion" ? abstractLogicalCalibrationCompletionItems : pilotSet === "classification_deduction" ? abstractLogicalCalibrationClassificationDeductionItems : abstractLogicalCalibrationSequenceMatrixItems;
  ...
  function select(optionId: AbstractLogicalCalibrationOptionId) {
    ...
    const selectionTimeMs = Math.max(0, performance.now() - questionStartedAtRef.current);
    const correct = optionId === item.correctOptionId;
    const response: AbstractLogicalCalibrationPilotResponse = {
      questionId: item.questionId,
      selectedOptionId: optionId,
      correct,
      selectionTimeMs,
  ...
  if (phase === "intro") {
    ...
    return <Shell right="Internal abstract & logical calibration"><section className="mx-auto max-w-6xl px-6 py-10 sm:px-8 sm:py-12"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only abstract & logical lab · v0.14</p><h1 className="mt-4 text-4xl font-semibold">Abstract & logical calibration</h1><p className="mt-5 max-w-3xl leading-relaxed text-[#9AA3B2]">All 20 Abstract & Logical calibration items have now passed device QA. The three authored sets remain available for re-checking and telemetry collection.</p><div className="mt-7 grid gap-4 lg:grid-cols-3"><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Sequences & matrices</div><div className="mt-2 text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 01–08. QA-cleared after final device re-check.</div><SecondaryButton className="mt-5" onClick={() => startRun("sequence_matrix")}>Run 01–08 again</SecondaryButton></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Classification, analogy & deduction</div><div className="mt-2 text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 09–16. QA-cleared after device review.</div><SecondaryButton className="mt-5" onClick={() => startRun("classification_deduction")}>Re-run 09–16</SecondaryButton></div><div className="rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Completion set</div><div className="mt-2 text-sm leading-relaxed text-[#AAB4C0]">4 items · reflection/rotation, rule machine, spatial outlier and integrated matrix. QA-cleared.</div><PrimaryButton className="mt-5" onClick={() => startRun("completion")}>Re-run 17–20</PrimaryButton></div></div><div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-5 text-sm leading-relaxed text-[#C8D2DD]">All 20 items are now QA-cleared. Re-runs are for regression checking and item-level timing/distractor observations; author timing ranges remain provisional.</div><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton>{priorRuns.length > 0 && <SecondaryButton onClick={downloadAbstractLogicalCalibrationPilotCsv}>Export {priorRuns.length} prior run{priorRuns.length === 1 ? "" : "s"}</SecondaryButton>}</div></Card></section></Shell>;
  }
  ...
  const times = responses.map((response) => response.selectionTimeMs);
  const spread = percentileRange(times);
  const debriefTitle = pilotSet === "completion" ? "Abstract & Logical completion-set re-check complete" : pilotSet === "classification_deduction" ? "Classification, analogy & deduction re-check complete" : "Sequence & matrix re-check complete";
...
function VerbalCalibrationSourceView({ source }: { source: VerbalCalibrationSource }) {
  return <div className="mb-6 rounded-2xl border border-white/5 bg-[#0F1216] p-4 sm:p-6">
    {source.eyebrow && <div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#6E7A88]">{source.eyebrow}</div>}
    {source.title && <h2 className="mt-2 text-xl font-semibold text-[#E5EAF0]">{source.title}</h2>}
    {source.paragraphs && <div className={`${source.title || source.eyebrow ? "mt-4" : ""} max-w-3xl space-y-4 text-[16px] leading-7 text-[#D5DCE4] sm:text-[17px] sm:leading-8`}>{source.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div>}
    {source.bullets && <ul className={`${source.title || source.eyebrow ? "mt-4" : ""} max-w-3xl space-y-3 pl-5 text-[16px] leading-7 text-[#D5DCE4] sm:text-[17px] sm:leading-8`}>{source.bullets.map((bullet, index) => <li key={index} className="list-disc pl-1 marker:text-[#5ED3F3]">{bullet}</li>)}</ul>}
    {source.sections && <div className={`${source.title || source.eyebrow || source.paragraphs || source.bullets ? "mt-5" : ""} grid gap-3 sm:grid-cols-2`}>{source.sections.map((section, index) => <div key={`${section.title}-${index}`} className="rounded-xl border border-white/5 bg-[#14181D] p-4"><h3 className="font-semibold text-[#DDE5EC]">{section.title}</h3>{section.paragraphs && <div className="mt-3 space-y-3 text-[15px] leading-6 text-[#C9D2DC] sm:text-[16px] sm:leading-7">{section.paragraphs.map((paragraph, paragraphIndex) => <p key={paragraphIndex}>{paragraph}</p>)}</div>}{section.bullets && <ul className="mt-3 space-y-2 pl-5 text-[15px] leading-6 text-[#C9D2DC] sm:text-[16px] sm:leading-7">{section.bullets.map((bullet, bulletIndex) => <li key={bulletIndex} className="list-disc pl-1 marker:text-[#5ED3F3]">{bullet}</li>)}</ul>}</div>)}</div>}
    {source.table && <div className={`${source.title || source.eyebrow || source.paragraphs || source.bullets || source.sections ? "mt-5" : ""}`}>
      {source.table.caption && <div className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#6E7A88]">{source.table.caption}</div>}
      <div className="overflow-hidden rounded-xl border border-white/10">
        <table className="w-full table-fixed text-left text-xs sm:text-sm"><thead className="bg-[#171C22] text-[#8D98A6]"><tr>{source.table.headers.map((header) => <th key={header} className="break-words px-2 py-2.5 font-semibold sm:px-3">{header}</th>)}</tr></thead><tbody>{source.table.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-t border-white/5">{row.map((cell, cellIndex) => <td key={`${rowIndex}-${cellIndex}`} className="break-words px-2 py-2.5 leading-5 text-[#D5DCE4] sm:px-3">{cell}</td>)}</tr>)}</tbody></table>
      </div>
    </div>}
  </div>;
}

function InternalVerbalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  const [phase, setPhase] = useState<"intro" | "question" | "debrief">("intro");
  const [questionIndex, setQuestionIndex] = useState(0);
  const [selectedOptionId, setSelectedOptionId] = useState<VerbalCalibrationOptionId | null>(null);
  const [responses, setResponses] = useState<VerbalCalibrationPilotResponse[]>([]);
  const [runStartedAt, setRunStartedAt] = useState<string | null>(null);
  const [pilotSet, setPilotSet] = useState<"core" | "applied" | "completion">("completion");
  const questionStartedAtRef = useRef(0);
  const activeItems = pilotSet === "core" ? verbalCalibrationCoreItems : pilotSet === "applied" ? verbalCalibrationAppliedItems : verbalCalibrationCompletionItems;
  const item: VerbalCalibrationPilotItem | undefined = activeItems[questionIndex];
  const currentResponse = item ? responses.find((response) => response.questionId === item.questionId) : undefined;
  const answered = Boolean(currentResponse);
  ...
  function startRun(set: "core" | "applied" | "completion" = "completion") {
    setPilotSet(set);
    setQuestionIndex(0);
    setSelectedOptionId(null);
    setResponses([]);
  ...
  function select(optionId: VerbalCalibrationOptionId) {
    if (answered || !item) return;
    const selectionTimeMs = Math.max(0, performance.now() - questionStartedAtRef.current);
    const correct = optionId === item.correctOptionId;
    const response: VerbalCalibrationPilotResponse = {
      questionId: item.questionId,
      selectedOptionId: optionId,
      correct,
      selectionTimeMs,
  ...
  function next() {
    if (!answered || !item) return;
    if (questionIndex === activeItems.length - 1) {
      saveVerbalCalibrationPilotRun({
        runId: id("verbal-cal"),
        pilotVersion: VERBAL_CALIBRATION_PILOT_VERSION,
        startedAt: runStartedAt ?? now(),
        completedAt: now(),
        responses,
      });
  ...
  if (phase === "intro") {
    const priorRuns = loadVerbalCalibrationPilotRuns();
    return <Shell right="Internal verbal calibration"><section className="mx-auto max-w-6xl px-4 py-8 sm:px-8 sm:py-12"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only verbal lab · v0.17</p><h1 className="mt-4 text-4xl font-semibold">Verbal calibration</h1><p className="mt-5 max-w-3xl leading-relaxed text-[#9AA3B2]">All 20 Verbal blueprint specifications are now QA-cleared after device review. The three sets remain available for regression checking and timing/distractor telemetry.</p><div className="mt-7 grid gap-4 lg:grid-cols-3"><div className="rounded-2xl border border-white/5 bg-[#111418] p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Core comprehension</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 01–08. QA-cleared after device review.</div><SecondaryButton className="mt-5" onClick={() => startRun("core")}>Re-run 01–08</SecondaryButton></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Extended verbal reasoning</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 09–16. QA-cleared after iPhone review.</div><SecondaryButton className="mt-5" onClick={() => startRun("applied")}>Re-run 09–16</SecondaryButton></div><div className="rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Verbal completion</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">4 items · claim versus evidence, reference resolution, policy interpretation and multi-paragraph synthesis.</div><PrimaryButton className="mt-5" onClick={() => startRun("completion")}>Re-run 17–20</PrimaryButton></div></div><div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-5 text-sm leading-relaxed text-[#C8D2DD]">All 20 Verbal specifications are authored and QA-cleared. Re-runs can now contribute item-level timing and distractor observations; author timing ranges remain provisional and are not learner standards.</div><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton>{priorRuns.length > 0 && <SecondaryButton onClick={downloadVerbalCalibrationPilotCsv}>Export {priorRuns.length} prior run{priorRuns.length === 1 ? "" : "s"}</SecondaryButton>}</div></Card></section></Shell>;
  }
  ...
  if (phase === "question") {
    const selectedCorrect = currentResponse?.correct ?? false;
    return <Shell right="Verbal calibration"><section className="mx-auto max-w-4xl px-4 py-7 sm:px-8 sm:py-10"><div className="mb-5 h-1.5 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-[#5ED3F3]/70" style={{ width: `${((questionIndex + 1) / activeItems.length) * 100}%` }} /></div><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-[#6E7A88] sm:text-sm sm:tracking-[0.22em]">{item.blueprintId} · {item.archetype}</p><h1 className="mt-2 text-2xl font-semibold sm:text-3xl">Verbal calibration item</h1></div><div className="text-sm text-[#8D98A6] sm:text-right">Question {questionIndex + 1} of {activeItems.length}<br/><span className="text-xs">author range {item.targetTimeRangeSec.minSec}–{item.targetTimeRangeSec.maxSec}s</span></div></div><Card className="p-4 sm:p-7"><VerbalCalibrationSourceView source={item.source}/><p className="text-lg font-medium leading-7 text-[#F4F6F8] sm:text-xl sm:leading-8">{item.stem}</p><div className="mt-6 grid gap-3">{item.options.map((option) => <button key={option.optionId} type="button" disabled={answered} onClick={() => select(option.optionId)} className={`rounded-2xl border p-4 text-left transition sm:p-5 ${selectedOptionId === option.optionId ? "border-[#5ED3F3]/60 bg-[#5ED3F3]/10" : "border-white/10 bg-[#111418] hover:border-[#5ED3F3]/40"}`}><div className="flex items-start gap-3"><span className="mt-0.5 shrink-0 font-semibold text-[#5ED3F3]">{option.optionId}</span><span className="text-[16px] leading-6 text-[#E5EAF0] sm:text-[17px] sm:leading-7">{option.text}</span></div></button>)}</div>{answered && <div className={`mt-5 rounded-2xl border p-5 ${selectedCorrect ? "border-[#38D39F]/40 bg-[#101D1A]" : "border-[#FFB86B]/40 bg-[#211813]"}`}><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-semibold">{selectedCorrect ? "Correct" : "Not quite"}</p><p className="mt-2 leading-relaxed text-[#C8D2DD]">{item.explanation}</p><p className="mt-3 text-sm text-[#8D98A6]">Selection latency: {formatSeconds(currentResponse?.selectionTimeMs ?? 0)} · diagnostic tag: {currentResponse?.misconceptionTag ?? "—"}</p></div><PrimaryButton className="shrink-0" onClick={next}>{questionIndex === activeItems.length - 1 ? "Complete pilot" : "Next item"}</PrimaryButton></div></div>}</Card></section></Shell>;
  }

  const correct = responses.filter((response) => response.correct).length;
  const times = responses.map((response) => response.selectionTimeMs);
  const spread = percentileRange(times);
  return <Shell right="Verbal calibration debrief"><section className="mx-auto max-w-5xl px-4 py-10 sm:px-8 sm:py-12"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Internal calibration result</p><h1 className="mt-3 text-4xl font-semibold">{pilotSet === "completion" ? "Verbal completion re-check finished" : pilotSet === "applied" ? "Verbal extended re-check finished" : "Verbal core re-check finished"}</h1><p className="mt-4 max-w-3xl leading-relaxed text-[#9AA3B2]">These are item-quality observations only. Author timing ranges remain provisional and are not learner standards.</p></div><Badge>{VERBAL_CALIBRATION_PILOT_VERSION}</Badge></div><div className="mt-8 grid gap-4 sm:grid-cols-3"><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Accuracy</div><div className="mt-3 text-3xl font-semibold">{correct}/{responses.length}</div></Card><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Median selection time</div><div className="mt-3 text-3xl font-semibold">{formatSeconds(median(times))}</div></Card><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">IQR</div><div className="mt-3 text-3xl font-semibold">{formatSeconds(spread.iqr)}</div></Card></div><Card className="mt-8"><h2 className="text-xl font-semibold">Item observations</h2><div className="mt-5 space-y-3">{responses.map((response, index) => { const responseItem = verbalCalibrationAllItems.find((candidate) => candidate.questionId === response.questionId); return <div key={`${response.questionId}-${index}`} className="rounded-2xl border border-white/5 bg-[#111418] p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><span className="font-medium text-[#DCE3EA]">{responseItem?.blueprintId ?? response.questionId}</span><span className="ml-2 text-sm text-[#8D98A6]">{responseItem?.archetype}</span></div><div className="text-sm text-[#AAB4C0]">{response.correct ? "Correct" : "Incorrect"} · {formatSeconds(response.selectionTimeMs)}</div></div><div className="mt-2 text-xs text-[#6E7A88]">{response.misconceptionTag}</div></div>; })}</div></Card><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><PrimaryButton onClick={() => setPhase("intro")}>Back to verbal lab</PrimaryButton><SecondaryButton onClick={() => startRun(pilotSet)}>Run this set again</SecondaryButton><SecondaryButton onClick={downloadVerbalCalibrationPilotCsv}>Export calibration CSV</SecondaryButton><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton></div></section></Shell>;
}

function InternalCalibrationFrameworkScreen({ onBack, onOpenReadiness, onOpenGearPilot, onOpenMechanicalPilot, onOpenNumericalPilot, onOpenAbstractLogicalPilot, onOpenVerbalPilot }: { onBack: () => void; onOpenReadiness: () => void; onOpenGearPilot: () => void; onOpenMechanicalPilot: () => void; onOpenNumericalPilot: () => void; onOpenAbstractLogicalPilot: () => void; onOpenVerbalPilot: () => void }) {
  ...
  return <Shell right="Internal calibration"><section className="mx-auto max-w-7xl px-6 py-10 sm:px-8 sm:py-12">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only framework</p><h1 className="mt-3 text-4xl font-semibold">General calibration blueprint</h1><p className="mt-4 max-w-3xl leading-relaxed text-[#9AA3B2]">The 80-item blueprint is now machine-readable. A specification can exist here without implying that the question has been authored, QA-cleared or empirically calibrated.</p></div><div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to dashboard</SecondaryButton><PrimaryButton onClick={onOpenReadiness}>Open calibration readiness</PrimaryButton><SecondaryButton onClick={onOpenGearPilot}>Open Gear Direction pilot</SecondaryButton><SecondaryButton onClick={onOpenMechanicalPilot}>Open mechanical calibration lab</SecondaryButton><SecondaryButton onClick={onOpenNumericalPilot}>Open numerical calibration lab</SecondaryButton><SecondaryButton onClick={onOpenAbstractLogicalPilot}>Open abstract/logical calibration lab</SecondaryButton><SecondaryButton onClick={onOpenVerbalPilot}>Open verbal calibration lab</SecondaryButton></div></div>
    ...
    <div className="mt-8 space-y-6">{generalCalibrationDomains.map((domain) => {
      const items = generalCalibrationBlueprints.filter((item) => item.domain === domain.id);
      return <details key={domain.id} open={domain.id === "verbal"} className="rounded-2xl border border-white/5 bg-[#151A21] p-5"><summary className="cursor-pointer font-semibold text-[#D9F8FF]">{domain.label} — {items.length} specifications</summary><div className="mt-5 overflow-x-auto"><table className="min-w-[980px] w-full text-left text-sm"><thead className="text-[#6E7A88]"><tr><th className="pb-3 pr-4">ID</th><th className="pb-3 pr-4">Subskill / archetype</th><th className="pb-3 pr-4">Target</th><th className="pb-3 pr-4">Level</th><th className="pb-3 pr-4">Implementation</th><th className="pb-3">QA</th></tr></thead><tbody>{items.map((item) => <tr key={item.blueprintId} className="border-t border-white/5 align-top"><td className="py-3 pr-4 text-[#8D98A6]">{item.blueprintId}</td><td className="py-3 pr-4"><div className="font-medium text-[#DDE3EA]">{item.subskill}</div><div className="mt-1 text-xs text-[#6E7A88]">{item.archetype}</div><div className="mt-1 max-w-xl text-xs leading-relaxed text-[#9AA3B2]">{item.itemBrief}</div></td><td className="py-3 pr-4 text-[#AAB4C0]">{item.targetTimeRangeSec.minSec}–{item.targetTimeRangeSec.maxSec}s</td><td className="py-3 pr-4 text-[#AAB4C0]">{item.difficulty}</td><td className="py-3 pr-4"><Badge>{item.implementationStatus === "pilot_live" ? "Pilot live" : item.implementationStatus === "qa_review" ? "QA review" : item.implementationStatus === "authoring" ? "Authoring" : "Blueprint only"}</Badge></td><td className="py-3 text-[#AAB4C0]">{item.qaStatus === "cleared" ? "Cleared" : item.qaStatus === "in_review" ? "In review" : "Not reviewed"}</td></tr>)}</tbody></table></div></details>;
    })}</div>
  </section></Shell>;
}


function InternalCalibrationReadinessScreen({ onBack, onOpenFramework, onOpenMechanicalPilot, onOpenNumericalPilot, onOpenAbstractLogicalPilot, onOpenVerbalPilot }: { onBack: () => void; onOpenFramework: () => void; onOpenMechanicalPilot: () => void; onOpenNumericalPilot: () => void; onOpenAbstractLogicalPilot: () => void; onOpenVerbalPilot: () => void }) {
  const rows = getCalibrationReadinessRows();
  const blueprintIds = rows.map((row) => row.blueprintId);
  const uniqueBlueprintIds = new Set(blueprintIds);
  const authoredLinks = getCalibrationAuthoredLinks();
  const authoredQuestionIds = authoredLinks.map((item) => item.questionId);
  const uniqueAuthoredQuestionIds = new Set(authoredQuestionIds);
  const frameworkIds = new Set(blueprintIds);
  const orphanedAuthoredItems = authoredLinks.filter((item) => !frameworkIds.has(item.blueprintId));
  const authoredLinked = rows.filter((row) => row.authoredLinked).length;
  const qaCleared = rows.filter((row) => row.qaStatus === "cleared").length;
  const pilotLive = rows.filter((row) => row.implementationStatus === "pilot_live").length;
  const metadataComplete = rows.filter((row) => row.coreMetadataComplete).length;
  const familyConsistent = rows.filter((row) => row.familyLinkConsistent).length;
  const technicalPilotReady = rows.length === 80 && authoredLinked === 80 && qaCleared === 80 && pilotLive === 80 && metadataComplete === 80 && familyConsistent === 80 && orphanedAuthoredItems.length === 0;
  const domainRows = generalCalibrationDomains.map((domain) => {
    const items = rows.filter((row) => row.domain === domain.id);
    return {
      ...domain,
      total: items.length,
      authored: items.filter((item) => item.authoredLinked).length,
      cleared: items.filter((item) => item.qaStatus === "cleared").length,
      live: items.filter((item) => item.implementationStatus === "pilot_live").length,
      metadata: items.filter((item) => item.coreMetadataComplete).length,
    };
  });
  const checks = [
    { label: "Blueprint count", value: `${rows.length}/80`, pass: rows.length === 80 },
    { label: "Unique blueprint IDs", value: `${uniqueBlueprintIds.size}/80`, pass: uniqueBlueprintIds.size === 80 },
    { label: "Authored linkage", value: `${authoredLinked}/80`, pass: authoredLinked === 80 },
    { label: "QA-cleared", value: `${qaCleared}/80`, pass: qaCleared === 80 },
    { label: "Pilot-live", value: `${pilotLive}/80`, pass: pilotLive === 80 },
    { label: "Core metadata complete", value: `${metadataComplete}/80`, pass: metadataComplete === 80 },
    { label: "Family linkage consistent", value: `${familyConsistent}/80`, pass: familyConsistent === 80 },
    { label: "Unique authored question IDs", value: `${uniqueAuthoredQuestionIds.size}/${authoredQuestionIds.length}`, pass: uniqueAuthoredQuestionIds.size === authoredQuestionIds.length },
    { label: "Orphaned authored items", value: String(orphanedAuthoredItems.length), pass: orphanedAuthoredItems.length === 0 },
  ];

  return <Shell right="Calibration readiness"><section className="mx-auto max-w-7xl px-4 py-8 sm:px-8 sm:py-12">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only · v0.18</p><h1 className="mt-3 text-4xl font-semibold">Calibration readiness</h1><p className="mt-4 max-w-3xl leading-relaxed text-[#9AA3B2]">The authoring phase is now consolidated into one 80-specification calibration bank. This screen checks whether the bank is structurally ready for a small technical pilot; it does not claim empirical difficulty, timing or selection validity.</p></div><div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to dashboard</SecondaryButton><SecondaryButton onClick={onOpenFramework}>Open blueprint framework</SecondaryButton><PrimaryButton onClick={downloadCalibrationReadinessCsv}>Export 80-item metadata CSV</PrimaryButton></div></div>

    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Authored-linked</div><div className="mt-3 text-3xl font-semibold">{authoredLinked}/80</div><div className="mt-2 text-sm text-[#9AA3B2]">Every blueprint has at least one authored calibration item or parallel form.</div></Card><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">QA-cleared</div><div className="mt-3 text-3xl font-semibold">{qaCleared}/80</div><div className="mt-2 text-sm text-[#9AA3B2]">Device and ambiguity review completed.</div></Card><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Core metadata</div><div className="mt-3 text-3xl font-semibold">{metadataComplete}/80</div><div className="mt-2 text-sm text-[#9AA3B2]">Archetype, format, difficulty, timing hypothesis and linked family present.</div></Card><Card className="p-5"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Empirical calibration</div><div className="mt-3 text-3xl font-semibold">0/80</div><div className="mt-2 text-sm text-[#9AA3B2]">Deliberately not claimed before real-user pilot data.</div></Card></div>

    <div className={`mt-6 rounded-2xl border p-6 ${technicalPilotReady ? "border-[#7FE0B8]/25 bg-[#7FE0B8]/5" : "border-[#FFB86B]/25 bg-[#211813]/60"}`}><div className="text-xs uppercase tracking-[0.18em] text-[#6E7A88]">Readiness interpretation</div><h2 className="mt-2 text-2xl font-semibold">{technicalPilotReady ? "Ready for a small technical pilot" : "Resolve structural checks before pilot handoff"}</h2><p className="mt-3 max-w-4xl leading-relaxed text-[#C8D2DD]">{technicalPilotReady ? "The bank has complete structural linkage and the device-QA statuses are reconciled. The next pilot should test telemetry capture, item IDs, exports, cross-device stability and unexpected interpretation problems—not establish cut-offs or validated timing norms." : "One or more structural checks remain incomplete. Use the integrity list below before collecting new pilot data."}</p></div>

    <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{domainRows.map((domain) => <div key={domain.id} className="rounded-2xl border border-white/5 bg-[#151A21] p-5"><div className="font-semibold text-[#D9F8FF]">{domain.label}</div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><div className="text-[#6E7A88]">Authored</div><div className="mt-1 text-lg font-semibold">{domain.authored}/{domain.total}</div></div><div><div className="text-[#6E7A88]">QA</div><div className="mt-1 text-lg font-semibold">{domain.cleared}/{domain.total}</div></div><div><div className="text-[#6E7A88]">Pilot-live</div><div className="mt-1 text-lg font-semibold">{domain.live}/{domain.total}</div></div><div><div className="text-[#6E7A88]">Metadata</div><div className="mt-1 text-lg font-semibold">{domain.metadata}/{domain.total}</div></div></div></div>)}</div>

    <Card className="mt-8"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Integrity audit</div><h2 className="mt-2 text-2xl font-semibold">Bank structure</h2></div><Badge>{GENERAL_CALIBRATION_BLUEPRINT_VERSION}</Badge></div><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{checks.map((check) => <div key={check.label} className="rounded-2xl border border-white/5 bg-[#111418] p-4"><div className="flex items-start justify-between gap-3"><div className="text-sm text-[#AAB4C0]">{check.label}</div><div className={check.pass ? "text-[#7FE0B8]" : "text-[#FFB3B3]"}>{check.pass ? "Pass" : "Check"}</div></div><div className="mt-2 text-xl font-semibold">{check.value}</div></div>)}</div></Card>

    <Card className="mt-8"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">What remains unvalidated</div><h2 className="mt-2 text-2xl font-semibold">Keep author hypotheses separate from evidence</h2><div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="font-medium text-[#D9F8FF]">Timing and difficulty</div><p className="mt-2 text-sm leading-relaxed text-[#9AA3B2]">Target time ranges, difficulty labels and rapid-recognition flags are author hypotheses until enough clean pilot observations exist across people and devices.</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="font-medium text-[#D9F8FF]">Selection validity</div><p className="mt-2 text-sm leading-relaxed text-[#9AA3B2]">No score, cut-off or probability of recruitment success should be inferred from this calibration bank.</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="font-medium text-[#D9F8FF]">Provider / employer mapping</div><p className="mt-2 text-sm leading-relaxed text-[#9AA3B2]">Provider and Employer Profiles remain separate from the stable item archetype layer. Profile relevance should continue to be backed by published evidence rather than baked into item correctness.</p></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5"><div className="font-medium text-[#D9F8FF]">Content freeze</div><p className="mt-2 text-sm leading-relaxed text-[#9AA3B2]">For the first technical pilot, avoid adding new archetypes unless a defect is found. A stable bank makes telemetry and regression findings interpretable.</p></div></div></Card>

    <Card className="mt-8"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Pilot handoff</div><h2 className="mt-2 text-2xl font-semibold">Next operational checks</h2><ol className="mt-5 space-y-3 text-sm leading-relaxed text-[#AAB4C0]"><li><span className="mr-2 text-[#6E7A88]">1.</span>Run a small technical pilot on phone and desktop with the bank frozen.</li><li><span className="mr-2 text-[#6E7A88]">2.</span>Confirm selection-time telemetry stops at answer selection, item IDs persist correctly and CSV exports reconcile with completed runs.</li><li><span className="mr-2 text-[#6E7A88]">3.</span>Record unexpected ambiguity, rendering defects and distractor patterns without changing timing targets mid-pilot.</li><li><span className="mr-2 text-[#6E7A88]">4.</span>Only after clean technical data, begin estimating empirical difficulty and response-time distributions.</li></ol><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onOpenMechanicalPilot}>Mechanical lab</SecondaryButton><SecondaryButton onClick={onOpenNumericalPilot}>Numerical lab</SecondaryButton><SecondaryButton onClick={onOpenAbstractLogicalPilot}>Abstract/logical lab</SecondaryButton><SecondaryButton onClick={onOpenVerbalPilot}>Verbal lab</SecondaryButton><PrimaryButton onClick={downloadCalibrationReadinessCsv}>Export readiness CSV</PrimaryButton></div></Card>
  </section></Shell>;
}
...
function getGearDiagramSpec(questionId: string): GearDiagramSpec {
  const specs: Record<string, GearDiagramSpec> = {
    ...
    "GEAR-FL-001": { teeth: [20, 20], yOffsets: [18, -18], driverIndex: 0, driverDirection: "anticlockwise" },
    "GEAR-FL-002": { teeth: [20, 20, 20], yOffsets: [-20, 20, -20], driverIndex: 0, driverDirection: "clockwise" },
    "GEAR-FL-003": { teeth: [20, 20, 20, 20], yOffsets: [22, -22, 22, -22], driverIndex: 0, driverDirection: "anticlockwise" },
    "GEAR-FL-004": { teeth: [20, 20, 20, 20, 20], yOffsets: [18, -18, 18, -18, 18], driverIndex: 0, driverDirection: "clockwise" },
    "GEAR-FL-005": { teeth: [20, 20, 20, 20, 20, 20], yOffsets: [-16, 16, -16, 16, -16, 16], driverIndex: 0, driverDirection: "anticlockwise" },
...
function GearAssessmentDebriefScreen({ journey, onWhy, onDashboard, onReviewIncorrect, onReviewAll, onStartPulleys }: { journey: MvpGuestJourney; onWhy: () => void; onDashboard: () => void; onReviewIncorrect: () => void; onReviewAll: () => void; onStartPulleys: () => void }) {
  const debrief = getLatestDebrief(journey);
  const rec = getCurrentRecommendation(journey);
...
function PulleyFundamentalsScreen({ journey, onSaveJourney, onComplete }: { journey: MvpGuestJourney; onSaveJourney: (journey: MvpGuestJourney) => void; onComplete: () => void }) {
  ...
  function persistProgress(nextProgress: ModuleProgress) {
    const nextJourney = journey.moduleProgress.some((item) => item.moduleProgressId === nextProgress.moduleProgressId)
      ? updatePulleyProgress(journey, nextProgress)
      : { ...journey, moduleProgress: [...journey.moduleProgress, nextProgress], updatedAt: now() };
    onSaveJourney(nextJourney);
  }
  ...
  function goNext() {
    if (miniCheck && !showFeedback) return;
    if (isFinalSection) { onComplete(); return; }
    persistProgress({ ...progress, currentSectionIndex: Math.min(progress.currentSectionIndex + 1, pulleyFundamentalsModule.sections.length - 1), updatedAt: now() });
  }
  ...
  return (
    <Shell>
      <section className="mx-auto max-w-5xl px-5 pb-24 pt-7 sm:px-8 sm:pb-12 sm:pt-12">
        <div className="mb-5 h-1.5 overflow-hidden rounded-full bg-white/5">
          <div className="h-full rounded-full bg-[#5ED3F3]/70 transition-all" style={{ width: `${((progress.currentSectionIndex + 1) / pulleyFundamentalsModule.sections.length) * 100}%` }} />
        </div>

        <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Pulley Fundamentals</p>
            <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">{pulleyFundamentalsModule.title}</h1>
            <p className="mt-2 text-sm text-[#9AA3B2] sm:text-base">{pulleyFundamentalsModule.subtitle}</p>
          </div>
          <Badge>Section {progress.currentSectionIndex + 1} of {pulleyFundamentalsModule.sections.length}</Badge>
        </div>
        ...
        <div ref={sectionTopRef} className="scroll-mt-4">
          <Card className="p-5 sm:p-8">
            ...
            {(section.sectionId === "pulley-fund-002" || section.sectionId === "pulley-fund-003" || section.sectionId === "pulley-fund-004") && <PulleyConceptDiagram />}

            {section.keyPoint && (
              <div className="mt-7 rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/10 p-5">
                <div className="text-xs uppercase tracking-[0.18em] text-[#6E7A88]">{section.sectionId === "pulley-fund-008" ? "Pocket principle" : "Key idea"}</div>
                <p className="mt-3 text-lg font-medium text-[#D9F8FF]">{section.keyPoint}</p>
              </div>
            )}
...
function PulleyReasoningReminder() {
  return (
    <div className="mb-5 rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 px-4 py-3 sm:px-5">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#5ED3F3]">Reasoning reminder</p>
      <p className="mt-1 text-sm leading-relaxed text-[#C8D2DD]">
        Find the moving load <span className="text-[#6E7A88]">→</span> trace the rope <span className="text-[#6E7A88]">→</span> count the supporting strands.
      </p>
    </div>
  );
}
...
function PulleyPracticeQuestionScreen({ journey, sessionId, questionIndex, onAnswer, questions, stage }: { journey: MvpGuestJourney; sessionId: string; questionIndex: number; onAnswer: (response: AssessmentResponse, final: boolean) => void; questions: MvpQuestion[]; stage: PulleyQuestionMode }) {
  ...
  function next() {
    if (!selectedOptionId) return;
    const response = createAssessmentResponse(sessionId, question, selectedOptionId, Date.now() - startedAt, false);
    onAnswer(response, questionIndex === questions.length - 1);
  }
      ...
      <section className="mx-auto max-w-5xl px-6 pb-20 pt-8 sm:px-8 sm:pb-12 sm:pt-10">
        ...
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold">Pulley reasoning</h1>
          </div>
          <div className="text-right text-sm text-[#8D98A6]">
        ...
        {stage !== "assessment" && <PulleyReasoningReminder />}

        <Card>
          <PulleyQuestionDiagram question={question} mode={stage} />
          <p className="mt-6 text-xl leading-relaxed text-[#F4F6F8]">{question.stem}</p>
          ...
          {immediate && showFeedback && (
            <div className={`mt-5 rounded-2xl border p-5 ${
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="shrink-0">
                  <PrimaryButton className="w-full sm:w-auto" onClick={next}>
                    {questionIndex === questions.length - 1 ? `Complete ${stage === "guided" ? "guided practice" : "independent practice"}` : "Next question"}
                  </PrimaryButton>
                </div>
              </div>
            </div>
          ...
          {!immediate && (
            <div className="mt-8 flex justify-end">
              <PrimaryButton disabled={!selectedOptionId} onClick={next}>
                {questionIndex === questions.length - 1 ? "Finish Pulley Check" : "Next question"}
              </PrimaryButton>
            </div>
          )}
        </Card>
...
function PulleyDebriefScreen({ journey, stage, onWhy, onDashboard, onNext, onReviewIncorrect, onReviewAll }: { journey: MvpGuestJourney; stage: "guided" | "independent" | "assessment"; onWhy: () => void; onDashboard: () => void; onNext?: () => void; onReviewIncorrect?: () => void; onReviewAll?: () => void }) {
  ...
  const canUseNext = stage === "guided" ? rec?.recommendationType === "begin_pulley_independent_practice" : stage === "independent" ? rec?.recommendationType === "begin_pulley_assessment" : stage === "assessment" ? rec?.recommendationType === "start_lever_fundamentals" : false;
    ...
    <Shell right={title}>
      <section className="mx-auto max-w-5xl px-6 pb-20 pt-8 sm:px-8 sm:pb-12 sm:pt-10">
        ...
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{title}</p>
            <h1 className="mt-3 text-3xl font-semibold">Lever reasoning</h1>
          </div>
          <div className="text-right text-sm text-[#8D98A6]">
            Question {questionIndex + 1} of {questions.length}
            <br />
        ...
        {stage !== "assessment" && <LeverReasoningReminder />}

        <Card>
          <LeverQuestionDiagram question={question} mode={stage} />
          <p className="mt-6 text-xl leading-relaxed text-[#F4F6F8]">{question.stem}</p>
          ...
          {!immediate && (
            <div className="mt-8 flex justify-end">
              <PrimaryButton disabled={!selectedOptionId} onClick={next}>
                {questionIndex === questions.length - 1 ? "Finish Lever Check" : "Next question"}
              </PrimaryButton>
            </div>
          )}
        </Card>
...
function VerbalDebriefScreen({ journey, stage, onWhy, onDashboard, onNext, onCoreProgress, onReviewIncorrect, onReviewAll }: { journey: MvpGuestJourney; stage: VerbalStage; onWhy: () => void; onDashboard: () => void; onNext?: () => void; onCoreProgress?: () => void; onReviewIncorrect?: () => void; onReviewAll?: () => void }) {
  const debrief = getLatestDebrief(journey);
  const rec = getCurrentRecommendation(journey);
...
function MixedMechanicalAssessmentIntroScreen({ onStart }: { onStart: () => void }) {
  return (
    <Shell>
      <section className="mx-auto flex min-h-[82vh] max-w-3xl items-center px-6 py-12 sm:px-8 sm:py-16">
        <Card className="text-center">
          <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Integrated evidence check</p>
          <h1 className="mt-5 text-4xl font-semibold">Mixed Mechanical Assessment</h1>
          <p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-[#9AA3B2]">
            Hydraulics, gears, pulleys and levers are mixed together without topic labels or immediate feedback. First identify the mechanical principle, then solve the problem.
          </p>
          <div className="mx-auto mt-9 grid max-w-2xl gap-3 sm:grid-cols-4">
            {["16 questions", "4 mechanical areas", "No immediate feedback", "Untimed in v1"].map((item) => (
              <div key={item} className="rounded-xl border border-white/5 bg-[#111418] p-4 text-sm text-[#AAB4C0]">{item}</div>
            ))}
          </div>
          <p className="mx-auto mt-7 max-w-xl text-sm leading-relaxed text-[#8D98A6]">
            Four questions from each mechanical area provide a balanced integrated check without turning the session into a long endurance test.
          </p>
          <div className="mt-10"><PrimaryButton onClick={onStart}>Start mixed assessment</PrimaryButton></div>
        </Card>
      </section>
    </Shell>
  );
...
function CorePathwaysCompleteScreen({ journey, onDashboard }: { journey: MvpGuestJourney; onDashboard: () => void }) {
  const progress = getDashboardProgress(journey);
  const checkCompleteCount = progress.filter((item) => item.status.toLowerCase().includes("complete")).length;
  const allChecksPresent = checkCompleteCount === 4;

  return (
    <Shell>
      <section className="mx-auto flex min-h-[82vh] max-w-5xl items-center px-6 py-12 sm:px-8 sm:py-16">
        <Card>
          <p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">{allChecksPresent ? "Core pathway milestone" : "Preparation review"}</p>
          <h1 className="mt-5 text-4xl font-semibold leading-tight">
            {allChecksPresent ? "You have completed the four core aptitude pathways." : "Review your progress across the four core pathways."}
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-relaxed text-[#9AA3B2]">
            Vivalsa uses your latest practice and check results to show where preparation is strongest and where another short block of work may help. These are preparation signals, not pass/fail judgements.
          </p>

          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {progress.map((item) => (
              <div key={item.name} className="rounded-2xl border border-white/5 bg-[#111418] p-5">
                <div className="text-sm font-semibold text-[#D9F8FF]">{item.name}</div>
                <div className="mt-3 text-3xl font-semibold">{item.value}</div>
                <div className="mt-2 text-sm text-[#AAB4C0]">{item.status}</div>
                <p className="mt-3 text-xs leading-relaxed text-[#6E7A88]">{item.detail}</p>
              </div>
            ))}
          </div>

          <div className="mt-8 rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/5 p-6">
            <div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">What next?</div>
            <p className="mt-3 leading-relaxed text-[#C8D2DD]">
              {allChecksPresent
                ? "The core journey is complete. Use the dashboard to revisit an area, repeat practice when useful, or later move into employer-specific and timed preparation."
                : "Return to your preparation plan. It will show which core pathway still needs a check or further consolidation."}
            </p>
          </div>

          <div className="mt-9">
            <PrimaryButton onClick={onDashboard}>View my preparation plan</PrimaryButton>
          </div>
        </Card>
      </section>
    </Shell>
...
export default function VivalsaPrototype() {
  ...
  function openCoreProgress() { setShowWhy(false); setScreen("core-pathways-complete"); }
  ...
  function continueAfterHydraulicIndependentPractice() {
    const rec = getCurrentRecommendation(journey);
    if (rec?.recommendationType === "start_gear_fundamentals") { openGearFundamentals(); return; }
    if (rec?.recommendationType === "continue_hydraulic_independent_practice") { setScreen("hydraulic-independent-practice-intro"); return; }
    if (rec?.recommendationType === "review_hydraulic_fundamentals") { openHydraulicFundamentals(); return; }
    setScreen("dashboard");
  }
