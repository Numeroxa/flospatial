...
function downloadVerbalCalibrationPilotCsv() {
  ...
  runs.forEach((run) => run.responses.forEach((response) => {
    ...
    rows.push([
      "APTESTA_VERBAL_CAL_EXPORT_V0_17", run.pilotVersion, run.runId, response.questionId, item?.blueprintId, item?.familyId, item?.archetype, item?.difficulty, item?.reasoningSteps, item?.targetTimeRangeSec.minSec, item?.targetTimeRangeSec.maxSec, response.selectedOptionId, response.correct, Math.round(response.selectionTimeMs), response.deviceClass, response.misconceptionTag, response.answeredAt,
    ]);
  }));
  const csv = rows.map((row) => row.map(csvCell).join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
...
function InternalVerbalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  ...
  if (phase === "intro") {
    ...
    return <Shell right="Internal verbal calibration"><section className="mx-auto max-w-5xl px-4 py-8 sm:px-8 sm:py-12"><Card><p className="text-sm uppercase tracking-[0.22em] text-[#6E7A88]">Tester-only verbal lab · v0.17</p><h1 className="mt-4 text-4xl font-semibold">Verbal calibration</h1><p className="mt-5 max-w-3xl leading-relaxed text-[#9AA3B2]">Verbal 01–16 are now QA-cleared after device review. This final four-item slice completes the authored Verbal blueprint with evidence discipline, reference resolution, policy interpretation and multi-paragraph synthesis.</p><div className="mt-7 grid gap-4 lg:grid-cols-3"><div className="rounded-2xl border border-white/5 bg-[#111418] p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Core comprehension</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 01–08. QA-cleared after device review.</div><SecondaryButton className="mt-5" onClick={() => startRun("core")}>Re-run 01–08</SecondaryButton></div><div className="rounded-2xl border border-white/5 bg-[#111418] p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">Pilot-live</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Extended verbal reasoning</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">8 items · blueprint 09–16. QA-cleared after your iPhone review.</div><SecondaryButton className="mt-5" onClick={() => startRun("applied")}>Re-run 09–16</SecondaryButton></div><div className="rounded-2xl border border-[#5ED3F3]/20 bg-[#5ED3F3]/5 p-5 sm:p-6"><div className="text-xs uppercase tracking-[0.16em] text-[#6E7A88]">New v0.17 QA set</div><div className="mt-2 text-xl font-semibold text-[#D9F8FF]">Verbal completion</div><div className="mt-2 max-w-3xl text-sm leading-relaxed text-[#AAB4C0]">4 items · claim versus evidence, reference resolution, policy interpretation and multi-paragraph synthesis.</div><PrimaryButton className="mt-5" onClick={() => startRun("completion")}>Start 4-item v0.17 QA set</PrimaryButton></div></div><div className="mt-6 rounded-2xl border border-white/5 bg-[#111418] p-5 text-sm leading-relaxed text-[#C8D2DD]">For this final device QA pass, focus on whether each question has only one defensible answer and whether the policy and two-paragraph synthesis items remain comfortable to read without losing the question/options on iPhone.</div><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><SecondaryButton onClick={onBack}>Back to framework</SecondaryButton>{priorRuns.length > 0 && <SecondaryButton onClick={downloadVerbalCalibrationPilotCsv}>Export {priorRuns.length} prior run{priorRuns.length === 1 ? "" : "s"}</SecondaryButton>}</div></Card></section></Shell>;
  }
...
function InternalCalibrationFrameworkScreen({ onBack, onOpenGearPilot, onOpenMechanicalPilot, onOpenNumericalPilot, onOpenAbstractLogicalPilot, onOpenVerbalPilot }: { onBack: () => void; onOpenGearPilot: () => void; onOpenMechanicalPilot: () => void; onOpenNumericalPilot: () => void; onOpenAbstractLogicalPilot: () => void; onOpenVerbalPilot: () => void }) {
  ...
  return <Shell right="Internal calibration"><section className="mx-auto max-w-7xl px-6 py-10 sm:px-8 sm:py-12">
    ...
    <div className="mt-6 rounded-2xl border border-[#5ED3F3]/15 bg-[#5ED3F3]/5 p-6"><div className="text-sm uppercase tracking-[0.18em] text-[#6E7A88]">Current rule</div><p className="mt-3 text-lg leading-relaxed text-[#D9F8FF]">Blueprint status, implementation status, QA status and empirical calibration are separate things. Gear Direction remains pilot-live. Gear Speed Ratio, Compound Gears, Open Belt Direction and Crossed Belt Direction are pilot-live after device QA. Mechanical extensions remain in QA review. Numerical 01–20 and Abstract & Logical 01–20 are now pilot-live after device QA. Verbal 01–16 are pilot-live after device QA. Verbal 17–20 are authored in v0.17 and remain in QA review.</p><p className="mt-3 text-sm leading-relaxed text-[#AAB4C0]">Framework: {GENERAL_CALIBRATION_BLUEPRINT_VERSION} · Live pilot families: {liveItems.length}</p></div>
