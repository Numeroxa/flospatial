...
function InternalAbstractLogicalCalibrationPilotScreen({ onBack }: { onBack: () => void }) {
  ...
  const times = responses.map((response) => response.selectionTimeMs);
  const spread = percentileRange(times);
  const debriefTitle = pilotSet === "completion" ? "Abstract & Logical v0.14 completion QA finished" : pilotSet === "classification_deduction" ? "Classification, analogy & deduction re-check complete" : "Sequence & matrix re-check complete";
